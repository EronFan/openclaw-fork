...
## OpenClaw 贡献项目团队调整（2026-04-12）

### 新架构
- **main（我）**：全面负责 OpenClaw 项目推进 + 写代码修 bug 提 PR（接管 aoao 所有职责）
- **xixi**：持续扫描调研，找 issue 候选并提交给 main
- **aoao**：已停止，所有职责并入 main

### 执行规则（2026-04-12 范总确认）
1. 项目由 main + xixi 两人 24/7 持续推进
2. 每完成一个 PR 立即向范总汇报
3. xixi 扫描到候选 issue → 提交给 main 处理
4. aoao 的 SOUL.md + AGENTS.md 技术文化已合并到 main 的工作规范

### aoao 工程师文化（已继承）
- 必须先复现再修，不在没看到问题的情况下写 fix
- 每条 PR 必须有测试覆盖
- 追求质量，不刷数量——烂 PR 被关闭等于浪费时间
- 修复流程：复现→定位根因→最小改动→写测试→pnpm test→PR
- commit 格式：`fix:` `feat:` `test:` `docs:` `chore:`
