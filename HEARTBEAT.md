...
## 晨报生成前置检查（必须执行）
**收到"晨报"类指令时，必须先执行完整 heartbeat 流程，再生成晨报：**

1. 先执行本文件规定的所有检查项（subagent 失败、xixi 扫描状态、git 未提交文件）
2. 更新 heartbeat-state.json
3. 执行自动 git commit（如有未提交文件）
4. 最后才生成晨报内容

**禁止**：收到"晨报"指令后直接生成内容，跳过 heartbeat 步骤。

**原因**：晨报是用户主动触发的时机，此时必须确保所有日常维护项（cron 状态、git commit）都被检查，不能遗漏。
...
## 记忆防丢失规则（2026-04-15 新增）
每次获取重要信息后，**立即**记录到 `memory/OPENCLAW-PROJECT.md`：
- GitHub issue/PR 状态变化 → 立即更新优先级表格
- 会议/对话中承诺的待办 → 立即记录到待办区
- 用户指示/偏好变化 → 立即更新 MEMORY.md
- 任何主动查询的信息 → 归拢到项目记忆再汇报，禁止裸查询裸回复

**禁止**查询后直接汇报而不记录到记忆系统。查询是为了行动，不是为了展示。
