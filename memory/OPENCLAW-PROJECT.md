...
| P60187 | [#69327](https://github.com/openclaw/openclaw/issues/69327) **🔴 S** sandbox.docker.env / setupCommand / workspace state 未可靠传播到 sandboxed subagent | 🔍 **aoao 搜索中**(runId 37ec889f 第133轮 20:33 CST) | aoao 搜索了相关源码文件（spawnedWorkspaceDir/sessions_spawn/sandbox bootstrap），尚未输出修复方案，需跟进 || P60188 | [#69326](https://github.com/openclaw/openclaw/issues/69326) **S** Webchat 发往 WeChat session 的回复未回投微信 | 🔍 **新发现**(方向1 GitHub 第133轮 20:26 CST) | 同一 session 从微信端发消息可正常回微信，但从 webchat 发起时 reply 只留在浏览器；疑似 `origin.provider=webchat` 覆盖 `deliveryContext.channel`；**建议确认 reply routing 优先级** |
| P60189 | [#69320](https://github.com/openclaw/openclaw/issues/69320) **S** Dreaming cron 被硬编码 600s timeout 误标 error，systemEvent 无法配置 timeout | 🔍 **新发现**(方向1 GitHub 第133轮 20:26 CST) | 实际 promotion 约 14 分钟完成但 cron 10 分钟先超时，监控长期误报；`cron edit --timeout-seconds` 不能作用于 systemEvent；**建议 aoao 接单** |
| P60190 | [#69304](https://github.com/openclaw/openclaw/issues/69304) **S regression** Telegram polling 在 gateway restart/update 后陷入 getUpdates 409 自冲突 | 🔍 **新发现**(方向1 GitHub 第133轮 20:26 CST) | 手工 `getUpdates` 在 gateway 停止时正常，一旦 Telegram provider 启动即自冲突；webhook 可绕过；评论指出与 #69254 recovery-aware restart 缺口同簇；**建议按跨 IM 渠道恢复逻辑处理** |
| P60191 | [#69303](https://github.com/openclaw/openclaw/issues/69303) **S** inbound dedupe 在 error path release 后可重放同一 message_id | 🔍 **新发现**(方向1 GitHub 第133轮 20:26 CST) | provider/auth/runtime 失败后仅 `releaseInboundDedupe` 不写 durable dedupe 标记，导致同一外部消息可重复注入 transcript；会重复扣 token、重复副作用；**根因清晰，值得接单** |
...
| P60197 | [heartbeat 2026-04-20 21:25 CST] xixi 高优先级建议跟进：#69294（ACP plugin crashes gateway on Windows 11）已派 aoao 快速研判 | ✅ **aoao 已完成**(runId f6ab5330) | 根因：捆绑插件 acpx 默认启用，Windows 11 上启动探针触发 Codex ACP (`npx @zed-industries/codex-acp`) 崩溃 (exit=3221225783)；已 patch `service.ts` 在 Windows 上跳过启动探针，改为首次使用时惰性检查；有回归测试；**注意**：仅缓解启动崩溃，未解决禁用默认插件的 UX 问题，后续可能需单独跟进取消 acpx 默认启用 |
| P60198 | [heartbeat 2026-04-20 21:26 CST] subagent 状态检查：最近 2 小时无 failed，`fix-69300-compaction-memory-flush` 已完成并产出 PR | ✅ **heartbeat 已检查** | `subagents(action=list, recentMinutes=120)` 返回 2 个 running、0 个 failed、1 个 done；`fix-69300-compaction-memory-flush` 已完成，session history 显示已推送修复+回归测试并创建 PR https://github.com/EronFan/openclaw-fork/pull/18；本轮无需失败重试，继续跟进 `fix-69296` 与 `aoao-fix-69294-from-heartbeat` |
| P60199 | [heartbeat 2026-04-20 21:29 CST] subagent / xixi 状态复检：最近 2 小时仍无 failed，xixi 21:00 CST 扫描正常 | ✅ **heartbeat 已检查** | `subagents(action=list, recentMinutes=120)` 返回 2 个 running、0 个 failed、0 个 recent done；`xixi-reports/latest-scan-report.md` 仍为 2026-04-20 21:00 CST，未超过 3 小时阈值；`OPENCLAW-PROJECT.md` 中未出现新的未处理 xixi 高优先级建议，当前继续等待 `fix-69296` 与 `aoao-fix-69294-from-heartbeat` 结果 |
| P60200 | [#69343](https://github.com/openclaw/openclaw/issues/69343) **S** Telegram CLI `message thread create` 错发 `thread-create` 导致 forum topic 创建失败 | ✅ **已调查—源码已修复** | Telegram runtime 已支持 `topic-create`，但通用 CLI bridge 未做 `thread-create -> topic-create` 归一化；100% 可复现，根因清晰，属 channel usability gap；**调查结果：源码已存在修复**（commit `2a40612058`：`fix: make telegram thread create use topic payload #54336`），`channel-actions.ts:171` 已有 `thread-create -> topic-create` 归一化；相关测试 7 passed；issue 可能为旧版本用户；无需重复派单 || P60201 | [#69341](https://github.com/openclaw/openclaw/issues/69341) **XS docs/CLI gap** `openclaw channels status` 文案提示 `--deep`，实际 flag 不存在 | 🔍 **新发现**(方向1 GitHub 第134轮 21:37 CST) | help / tip 与 CLI 实现不一致；不是 crash，但会误导排障；可用 1 个 docs/CLI 小修补齐 |
| P60202 | contributor-area: `src/browser/server-context.ts` / remote-tab-ops ↔ [#66809](https://github.com/openclaw/openclaw/issues/66809) browser loopback CDP path blocked by policy | 🔍 **新发现**(方向3 文件区 第134轮 21:37 CST) | 末位 contributor `pandego` 最近提交集中在 browser `server-context*`；与 loopback CDP / local managed browser 路径高度同区，issue 仍 open 未见认领 |
...
| P60207 | [#69360](https://github.com/openclaw/openclaw/issues/69360) **🔴 S regression** isolated cron `agentTurn` 成功执行却被标记 `Agent couldn't generate a response` | 🔍 **新发现**(方向1 GitHub 第135轮 22:47 CST) | cron log 已显示 `output_tokens=694`、`stopReason=stop`，但因 `payloads=0` 走 incomplete-turn 错误分支；`src/cron/isolated-agent/run.ts` / `pi-embedded-runner` 同区，**建议优先接单** |
| P60208 | [#69359](https://github.com/openclaw/openclaw/issues/69359) **🟠 S beta-blocker** Webchat 图片上传发 CDN 临时 URL，模型拿不到真实图片 | 🔍 **新发现**(方向1 GitHub 第135轮 22:47 CST) | Control UI 把图片变成 MiniMax CDN 临时链接而不是 inline/local file，视觉模型分析失败；与 #69358 同簇，**建议合并处理图片链路** |
| P60209 | [#69358](https://github.com/openclaw/openclaw/issues/69358) **🟠 S regression** Control UI 纯图片消息被当作“没有文本” | 🔍 **新发现**(方向1 GitHub 第135轮 22:47 CST) | 报告直接指向 `MediaPath/MediaPaths` 未注入 sessionCtx；与 #69359 共同说明 webchat 图片链路回归，**建议合并追踪** |
| P60210 | [#69353](https://github.com/openclaw/openclaw/issues/69353) **M** `context1m` 对非 `anthropic` provider 无效 | 🔍 **新发现**(方向1 GitHub 第135轮 22:47 CST) | `isAnthropic1MModel()` 硬编码 provider=anthropic，导致 github-copilot 等代理 Anthropic 模型无法吃到 1M context；**中优先级** |
| P60211 | contributor-area: `src/agents/pi-embedded-runner/*` / `src/cron/isolated-agent/run.ts` ↔ [#69360](https://github.com/openclaw/openclaw/issues/69360) | 🔍 **新发现**(方向3 文件区 第135轮 22:47 CST) | 末位 contributor `davidrudduck` 最近 20 条 commit 高度集中在 isolated cron / embedded runner / compaction 路径；与本轮最高优先级 bug 完全同区 |
| P60212 | contributor-area: `src/browser/server-context.ts` ↔ [#66809](https://github.com/openclaw/openclaw/issues/66809) browser loopback CDP / policy path | 🔍 **新发现**(方向3 文件区 第135轮 22:47 CST) | 末位 contributor `pandego` 最近改动集中在 browser `server-context*`；与既有 open bug #66809 同区，继续保留为 contributor-area 候选 |
| P60213 | contributor-area: `extensions/ollama/*` / `src/agents/ollama-*` ↔ [#61223](https://github.com/openclaw/openclaw/issues/61223) Ollama thinking tokens ignored | 🔍 **新发现**(方向3 文件区 第135轮 22:47 CST) | 末位 contributor `BruceMacD` 最近改动集中在 Ollama provider / stream / setup；与既有 open bug #61223 同区，值得继续盯 |
| P60214 | [方向4 第135轮 22:47 CST] 已追踪 PR 暂无 maintainer 新评论；heartbeat-state 中 fork PR #16/#17 仍 open 无变化 | 👀 **状态检查** | 最近 2 小时未发现值得动作的 maintainer 新反馈；`OPENCLAW-PROJECT.md` 已追踪高优项本轮也无新的 upstream 状态跳变 |
| P60215 | [#69360](https://github.com/openclaw/openclaw/issues/69360) **🔴 S regression** isolated cron `agentTurn` 成功执行却被标记 `Agent couldn't generate a response` | ✅ **PR#69367 已创建** | 根因：空 assistantTexts 导致 payload builder 不回退到 final_answer，payloads=0 触发 incomplete-turn；修复：`payloads.ts` 忽略空白 assistantTexts + `run.ts` 有 finalAssistantVisibleText 时合成最小文本 payload；测试 64 passed；commit `6b84577ec2`；**runId a70f285e** |
| P60216 | [cron 2026-04-20 22:46 CST] 反馈检查 + xixi 新报告处理完成 | ✅ **已处理** | `gh api` 确认 #54952 / #54964 仍是 0 comments；PR #55008 / #55013 无新 feedback；xixi 22:47 CST 新报告为全新结论，当前优先级已包含 #69360 / #69359 / #69358，已派 aoao 修复 #69360，并更新 `last-processed-report.md` |
...
| P60150 | [#66692](https://github.com/openclaw/openclaw/pull/66692) audio transcription allowPrivateNetwork | 🔲 待 review | PR 已开，mergeable，建议 review |
...
| P60121 | [#66647](https://github.com/openclaw/openclaw/issues/66647) **🔥 S regression** Telegram 二进制文件 caption 注入 LLM prompt，导致 token 爆炸 | ✅ **PR [#66663](https://github.com/openclaw/openclaw/pull/66663) 已合并** | `.mobi` 电子书文件 → 460,506 tokens → context overflow；根因：`getTelegramTextParts()` 无 binary content 检测；LINE 渠道有正确实现可参照；**作者已给完整 fix，已合并** |
...
| P60129 | [#66601](https://github.com/openclaw/openclaw/issues/66601) **🔥 regression blocker** `lossless-claw` context engine 工厂返回 invalid ContextEngine，系统彻底不可用 | ✅ **PR [#66680](https://github.com/openclaw/openclaw/pull/66680) 已创建** | v2026.4.14 regression；根因：`describeResolvedContextEngineContractError()` 中 `infoId !== engineId` 校验（commit 2677f7cf14）误杀外部插件；修复：删除该校验 + 删除对应测试用例；`pnpm test -- src/context-engine/context-engine.test.ts` → 35 passed |
| P60130 | [#66669](https://github.com/openclaw/openclaw/issues/66669) **S** Control UI (webchat) 不渲染用户发送的图片附件，只显示空 bubble | ✅ **PR [#66703](https://github.com/openclaw/openclaw/pull/66703) 已创建** | 前端 UI regression；user message 组件未渲染 `attachments` 数组；AI 消息图片正常；修复：extractImages() 增加 MediaPath/MediaPaths 字段处理；renderMessageImages() 增加 gateway URL 构建；**EronFan 修复** |
| P60131 | [#66668](https://github.com/openclaw/openclaw/issues/66668) **S** `plugins uninstall` 对 `--link` 或手动安装的插件不删除文件 | ✅ **PR [#66704](https://github.com/openclaw/openclaw/pull/66704) 已创建** | 功能缺口；`uninstall` 只清理 npm 安装文件；修复：区分 linked install（installPath===sourcePath）和普通 path install（installPath!==sourcePath），前者跳过文件删除，后者正常删除；测试 36 passed；**EronFan 修复** |
| P60132 | [#66665](https://github.com/openclaw/openclaw/issues/66665) **S** Harden MCP loopback request validation (maintainer PR) | 🔍 新发现(方向1 GitHub 第126轮 00:34 CST) | maintainer 安全 PR；MCP loopback 请求验证加固；**建议跟进 merge 状态** |
| P60133 | [#66664](https://github.com/openclaw/openclaw/issues/66664) **S** guard Anthropic Messages max tokens (maintainer PR) | 🔍 新发现(方向1 GitHub 第126轮 00:34 CST) | maintainer bugfix PR；**建议跟进 merge 状态** |
| P60134 | [#66671](https://github.com/openclaw/openclaw/issues/66671) **XS** `session-status-tool.ts` 使用 source-tree-relative import，bundling 后崩溃 | 🔍 新发现(方向1 GitHub 第126轮 00:34 CST) | `importRuntimeModule(import.meta.url, [...])` 打包后路径错误；**PR #66671 已开**；建议确认 merge 窗口 |
| P60135 | [#66693](https://github.com/openclaw/openclaw/issues/66693) **S regression** Onboarding `TypeError: Cannot read properties of undefined (reading 'trim')` (2026.4.14) | 🔥 **PR #66653 已开**（方向1 GitHub 第127轮 01:11 CST） | 两个独立 reporter 确认；onboarding channel selection 后 `.trim()` on undefined；**PR #66653（mm1ord）已修同根因**；建议立即 review + approve |
| P60136 | [#66681](https://github.com/openclaw/openclaw/issues/66681) **S** Health-monitor causes gateway crash: `Cannot read properties of undefined (reading 'info')` | 🔥 **aoao 派出**（方向1 GitHub 第127轮 01:11 CST） | `log.info?.()` 保护了 `info` 但没保护 `log`；应改为 `log?.info?.()`；**1行 fix，root cause 极清晰**；建议立即接单 |
| P60137 | [#66679](https://github.com/openclaw/openclaw/issues/66679) **S regression** Telegram+Slack chat-triggered tools/actions 完全不执行 (2026.4.14 a88c6f0) | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | agent→channel action dispatch 路径静默失败；channel probe 正常但工具不触发；跨 Telegram+Slack 两个主流渠道；**建议 aoao 接单** |
| P60138 | [#66691](https://github.com/openclaw/openclaw/issues/66691) **S regression** `models.providers.*.request.allowPrivateNetwork` 不再对 audio transcription 生效 (v2026.4.14) | 🔍 **PR #66692 已开**（方向1 GitHub 第127轮 01:11 CST） | 根因：① `resolveProviderExecutionContext` 未合并 provider-level request config ② `resolveProviderRequestPolicyConfig` 忽略 `params.request?.allowPrivateNetwork`；**PR #66692（jhsmith409）已开**；Greptile P2：无 regression test |
| P60139 | [#66690](https://github.com/openclaw/openclaw/issues/66690) **S** Sandboxed agent cannot reach browser CDP — `127.0.0.1` hardcoded in `ensureSandboxBrowser` | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | Docker sandbox agent 无法访问 host loopback CDP；需用容器内部 DNS 名；文档推荐的 sandbox+browser 组合完全不可用；**建议 aoao 接单** |
| P60140 | [#66674](https://github.com/openclaw/openclaw/issues/66674) **S** openai-codex/gpt-5.4 CLI 返回 HTML rawError 但 surface 为 "DNS lookup failed" (2026.4.14) | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | HTML provider 错误被误分类为 DNS 异常；`rawErrorPreview` 含 HTML；所有 Codex 用户完全无法用；**建议 aoao 接单** |
| P60141 | [#66688](https://github.com/openclaw/openclaw/issues/66688) **S regression** `memory index` fails with `Unknown memory embedding provider: ollama` | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | ollama 作为 memory embedding provider 不被识别；regression；**建议 aoao 接单** |
| P60142 | [#66686](https://github.com/openclaw/openclaw/issues/66686) **S** Memory search over-generalization: 系统状态验证被强制走 memory_search | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | `Before answering anything about prior work...` 规则过宽；系统状态验证应直接调 API；system prompt 改1-2行；**建议 aoao 接单** |
| P60143 | [#66683](https://github.com/openclaw/openclaw/issues/66683) **S** Subagent completions resolve before parent reply delivery succeeds | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | 生命周期 ownership 问题；可能导致 duplicate completion 或 silent drop；与 jalehman 文件区重叠；**建议 aoao 接单** |
| P60144 | [#66682](https://github.com/openclaw/openclaw/issues/66682) **S regression** Telegram `commands.native` 不注册 setMyCommands (2026.4.14) | 🔍 新发现（方向1 GitHub 第127轮 01:11 CST） | setup 代码存在但从未执行；无任何日志/调用；与 jalehman 文件区重叠；**建议 aoao 接单** |
| P60145 | [#66689](https://github.com/openclaw/openclaw/pull/66689) **🔒 SECURITY BLOCK** workspace media paths — Symlink escape allows arbitrary host file read | ✅ **已合并**（方向1 GitHub 第127轮 01:11 CST） | aisle-research-bot 报告 symlink escape → arbitrary host file read；`isPathInside` 检查在 `realpath` 前信任 path → 符号链接逃脱；**已修复并合并** |
...
| P59908 | [方向4] 已追踪 PR 本轮无 maintainer 新评论；状态变化是 [#64657](https://github.com/openclaw/openclaw/pull/64657) 作者补充与 #54840 的关系说明，[#65021](https://github.com/openclaw/openclaw/pull/65021) 持续有自动 review，[#65016](https://github.com/openclaw/openclaw/pull/65016) 仍 open，[#65144](https://github.com/openclaw/openclaw/pull/65144)/[#65146](https://github.com/openclaw/openclaw/pull/65146) 继续 closed | 👀 状态变化(方向4 第98轮 13:45 CST) | 本轮没有 maintainer 人工新反馈，但追踪 PR/关闭状态有更新，需继续盯流程窗口与后续 reopen / merge 机会 |
| P59900 | [#65177](https://github.com/openclaw/openclaw/issues/65177) `openclaw doctor --fix` 未迁移 Feishu `botName -> name`，升级 2026.4.11 后 gateway 启动失败 | 🔍 新发现(方向1 GitHub 第97轮 12:41 CST) | 明确 migration gap；老配置仍常见，且 `doctor --fix` 本应兜底却失效；直接阻断 Feishu 用户升级；可修性 S，值得 aoao 处理 |
| P59901 | [#65168](https://github.com/openclaw/openclaw/issues/65168) node.invoke 在 node pairing approval 前仍可调用 | 🔍 新发现(方向1 GitHub 第97轮 12:41 CST) | **高风险安全问题**；报告给出完整 CVSS 和源码链路，`operator.write` 可能在未完成 node.pair.approve 前调用声明命令，macOS/Linux 上甚至可碰到 `system.run`；需高度关注，建议安全优先级提升 |
| P59902 | [#65164](https://github.com/openclaw/openclaw/issues/65164) Windows 下 gateway 进程内所有 outbound HTTP 失败，但 WebSocket 正常 | 🔍 新发现(方向1 GitHub 第97轮 12:41 CST) | 范围大且诡异；Telegram/Discord REST、模型预热、内部 127.0.0.1 握手全挂，独立 node `fetch` 正常，疑似 gateway 初始化污染全局 HTTP stack；影响多渠道 Windows 用户 |
| P59903 | [方向4] PR [#65144](https://github.com/openclaw/openclaw/pull/65144) / [#65146](https://github.com/openclaw/openclaw/pull/65146) 被 barnacle 因“active PR > 10”自动关闭 | 👀 状态变化(方向4 第97轮 12:41 CST) | 不是代码质量问题，`EronFan` 已 approve；属于流程阻塞，需要等活跃 PR 数下降后 reopen / resubmit；两条修复分别对应 context overflow spinner 与 Gmail duplicate watcher |
| P59895 | [#65141](https://github.com/openclaw/openclaw/issues/65141) iMessage default + named account 双 watcher 导致重复回复 | 🔍 新发现(方向1 GitHub 第96轮 11:33 CST) | 明确行为 bug; 同一 local Messages 源被拉起两个 `imsg rpc` watcher, 单条 inbound 触发双 reply; 复现条件清晰(`channels.imessage.accounts` 同时存在 `default` 和命名账号); 可修性 S-M |
| P59896 | [#65136](https://github.com/openclaw/openclaw/issues/65136) gateway restart 后 task/subagent 仍卡在 running, maintenance 不回收 | 🔍 新发现(方向1 GitHub 第96轮 11:33 CST) | crash-recovery / task-state bug; `/reset` 或 gateway restart 杀掉子进程后 `openclaw tasks audit` 持续报 stale_running, `maintenance --apply` 无效; 影响任务审计与长期稳定性 |
| P59897 | contributor-area: `src/gateway/control-plane-rate-limit*` ↔ [#64911](https://github.com/openclaw/openclaw/issues/64911) gateway 提前 ready 但 WS control plane 尚不可用 | 🔍 新发现(方向3 文件区 第96轮 11:33 CST) | 来自末段 contributor `lml2468` 最近活跃文件; 与 restart 后 CLI RPC / cron run 握手超时问题直接同区, 未见认领, 值得继续盯 |
| P59898 | contributor-area: `src/gateway/server-maintenance.ts` / `src/gateway/server-methods/nodes.ts` ↔ [#64984](https://github.com/openclaw/openclaw/issues/64984) reconnect 后 MCP 子进程不清理导致 OOM | 🔍 新发现(方向3 文件区 第96轮 11:33 CST) | 来自末段 contributor `lml2468` 活跃文件区; 与 gateway reconnect / maintenance 路径高度相关, 属于高风险稳定性 bug, 目前 open 且 0 评论 |
| P59899 | [方向4] 已追踪 PR 暂无 maintainer 新评论；#64681 / #65016 / #65012 仍 open | 👀 状态检查(方向4 第96轮 11:33 CST) | 本轮仅确认状态无新转折: PR #64681 updated 23:43 UTC, #65016 updated 22:06 UTC, #65012 updated 21:45 UTC; 新发现 issue #65076/#65078/#65082/#65086 仍均 open |
| P59909 | [#65210](https://github.com/openclaw/openclaw/issues/65210) **CRITICAL REGRESSION** Control UI token usage shows N/A + auto-compression fails since 2026.4.9 | 🔥 **aoao 已派出**(方向1 GitHub 第99轮 14:51 CST) | **最高优先级**；三重奏：(1) Sessions 视图 Tokens 列显示 "unknown/262k (?%)"; (2) Usage 页面 token 统计完全缺失; (3) auto-compression 失效导致 context overflow；根因：OpenClaw 无法正确解析 API 响应的 `usage` 字段，级联导致 token 计数、自动压缩全线崩溃；影响所有 2026.4.9+ 用户；runId 03323276；**建议优先接单** |
| P60005 | [#65584](https://github.com/openclaw/openclaw/issues/65584) **S** cron CLI commands succeed but hang instead of exiting cleanly | 🔥 **aoao 已派出**(方向1 GitHub 第106轮 07:39 CST) | 根因在 CLI/gateway transport teardown；清晰可本地复现；runId af2e0e6e |
| P60081 | [#66135](https://github.com/openclaw/openclaw/issues/66135) **🔥 regression** Background exec exit 不唤醒 session、cron --at jobs 静默消失、Claude CLI --print 输出截断（2026.4.10） | 🔥 **aoao 已派出**(方向1 GitHub 第122轮 15:19 CST) | 三重 regression：background exec exit 不唤醒 session、cron --at jobs 静默消失、Claude CLI --print 输出被截断；2026.4.10 引入；**runId 96163316** |
| P60082 | [#66393](https://github.com/openclaw/openclaw/issues/66393) **S regression** Image viewing not working in webchat | 🔥 **aoao 已派出(runId 1cded531)**(方向1 GitHub 第122轮 15:19 CST) | webchat 图片粘贴/附件无法被 assistant 分析；**runId 1cded531** |
| P60083 | [#66391](https://github.com/openclaw/openclaw/issues/66391) **S regression** TUI tool calls fail "session ID openclaw-tui does not exist" on local Ollama | 🔥 **aoao 已派出(runId fa5a5f46)**(方向1 GitHub 第122轮 15:19 CST) | TUI session 路由问题，本地模型工具调用 100% 失败；**runId fa5a5f46** |
| P60086 | [#66421](https://github.com/openclaw/openclaw/issues/66421) **🔥 CRITICAL regression** AutoClaw update overwrites/loses Agent workspace files (SOUL/MEMORY/IDENTITY) | 🔥 **aoao 已派出(runId 22eba75e)**(方向1 GitHub 第123轮 17:27 CST) | 更新过程静默覆盖/删除用户个性化文件；所有 AutoClaw update 用户受影响；regression；0评论未认领；**最高优先级数据丢失 bug**；runId 22eba75e-e361-4fda-8110-f85acf267b80 |
| P60087 | [#66443](https://github.com/openclaw/openclaw/issues/66443) **S regression** Overflow recovery duplicates role=user messages in session JSONL, amplifying transcript growth | 🔥 **aoao 已派出(runId f98b2500)**(方向1 GitHub 第123轮 17:27 CST) | context overflow 恢复时重复追加 user 消息导致 session JSONL 无限膨胀；与 #66441 同根；**runId f98b2500-96af-4996-b018-7473309e5637** |
| P60088 | [#66441](https://github.com/openclaw/openclaw/issues/66441) **S regression** Context overflow recovery may re-append identical user prompts (heartbeat polls), inflating session transcripts | 🔍 建议合并 #66443 追踪(方向1 GitHub 第123轮 17:27 CST) | 与 #66443 同 issue 两种表述；heartbeat poll 导致重复追加；**建议派 aoao** |
| P60089 | [#66436](https://github.com/openclaw/openclaw/issues/66436) **S regression** ThrottleInterval 1s causes 37 MB error logs on missing config | 🔥 **aoao 已派出(runId 25831b4c)**(方向1 GitHub 第123轮 17:27 CST) | ThrottleInterval 配置缺失时 1s 间隔刷屏错误日志；37MB 噪音日志；runId 25831b4c-5a4b-4a94-9acd-a0b16e1c5e47 |
| P60090 | [#66433](https://github.com/openclaw/openclaw/issues/66433) **S regression** Upgrading openclaw removes node-llama-cpp, breaking local embeddings | 🔥 **aoao 已派出(runId 89cd476d)**(方向1 GitHub 第123轮 17:27 CST) | 升级后 node-llama-cpp 丢失，本地 embeddings 功能破坏；runId 89cd476d-4284-4912-be7e-b7ebc35197fd |
| P60091 | [#66430](https://github.com/openclaw/openclaw/issues/66430) **M regression** Dreaming REM phase causes cross-agent context pollution when agents share workspace | 🔍 新发现(方向1 GitHub 第123轮 17:27 CST) | regression；Dreaming REM 阶段污染共享 workspace 的跨 agent context；建议 aoao 调研 dreaming phase isolation |
| P60092 | [#66424](https://github.com/openclaw/openclaw/issues/66424) **S regression** github-copilot/gpt-5.4 intermittently fails with 401 across sessions/agents | 🔥 **aoao 已派出(runId 1a922637)**(方向1 GitHub 第123轮 17:27 CST) | 多 session/agent 下 401 间歇性失败；runId 1a922637-b825-4aec-8768-957264d40f3f |
| P60093 | [#66423](https://github.com/openclaw/openclaw/issues/66423) **S regression** only `<tool_call>` tags replied as text, without any tool call action | 🔥 **aoao 已派出(runId 58667cef)**(方向1 GitHub 第123轮 17:27 CST) | tool_call 标签只作为文本回复而非实际工具调用；100% 可复现；runId 58667cef-44a6-49e7-8011-ed629033d355 |
| P60094 | [方向4 第123轮 17:27 CST] **PR #66458 merged** — huntharo fix(media-understanding): auto-upgrade provider HTTP helper to trusted env proxy mode | ✅ **已 merge (09:29 UTC)** | huntharo author → maintainer 快速 merge（<5分钟）；本轮最大交付成果 |
| P60101 | [#66573](https://github.com/openclaw/openclaw/issues/66573) **S** chat.history limit=2000 exceeds server maximum of 1000, no retry — 上下文静默丢失 | 🔥 **aoao 已派出(runId 792910f4)**(方向1 GitHub 第124轮 21:50 CST) | 根因：客户端写死 limit=2000，服务器上限 1000，INVALID_REQUEST 后静默跳过；S级，5行可修；**runId 792910f4-3f94-41db-add1-4b76d3ca0ee5** |
| P60102 | [#66564](https://github.com/openclaw/openclaw/issues/66564) **S regression** Control UI feels significantly laggier in 2026.4.12 | 🔥 **aoao 已派出(runId 9032d4b2)**(方向1 GitHub 第124轮 21:50 CST) | node.list 每5s轮询、logs 每2s、debug 每3s；相关 PR #66575 正在修复但未完全解决；**runId 9032d4b2-ef0e-49e5-a9ef-9dd50b8e6105** |
| P60103 | [#66561](https://github.com/openclaw/openclaw/issues/66561) **S** openai-codex SSE stream begins but embedded run aborts locally, surfaced as timeout (408) | 🔥 **aoao 已派出(runId 06caa6c3)**(方向1 GitHub 第124轮 21:50 CST) | mitmproxy 抓包证明 upstream 已开始响应但 client abort 误判为 timeout；S级；**runId 06caa6c3-84a0-4816-991a-353e28630653** |
| P60104 | [#66558](https://github.com/openclaw/openclaw/issues/66558) **S** sessions_spawn/sessions_send not available in main agent despite tools.profile: "coding" | 🔥 **aoao 已派出(runId f6e3e92f)**(方向1 GitHub 第124轮 21:50 CST) | 工具配置解析问题；与 #42726/#56208 同簇；**runId f6e3e92f-89a5-4192-be59-8e3fc92c831c** |
| P60105 | [#66553](https://github.com/openclaw/openclaw/issues/66553) **M** Race condition — voice message delivery fires before STT transcription is available | 🔍 新发现(方向1 GitHub 第124轮 21:50 CST) | WhatsApp voice → StoreMessage 立即提交 transcription=NULL → agent 读到空 transcription → STT 2-5s 后完成但 agent turn 已结束；100% 可复现；**建议 aoao 接单但先确认根因路径** |
| P60106 | [#66535](https://github.com/openclaw/openclaw/issues/66535) **S** /compact command cannot be canceled while in progress | 🔥 **aoao 已派出(runId 48c1df8f)**(方向1 GitHub 第124轮 21:50 CST) | /compact 无法取消，用户体验影响大；**runId 48c1df8f-d6d1-4c53-b783-0ca97cefab09** |
| P60107 | [#66522](https://github.com/openclaw/openclaw/issues/66522) **M** Session index rebuilds on restart, session history completely lost (session files exist but sessions.json rebuilt → invisible) | 🔍 新发现(方向1 GitHub 第124轮 21:50 CST) | session 文件存在但 sessions.json 重建后 sessions_history 读不到；M级；**建议 aoao 接单** |
| P60108 | [#66500](https://github.com/openclaw/openclaw/issues/66500) **S** Feishu Chinese filenames garbled（UTF-8 → Latin-1 mojibake） | 🔍 待确认(方向1 GitHub 第124轮 21:50 CST) | 可能在 PR 覆盖区，需确认；**待确认 PR** |
| P60109 | [#66540](https://github.com/openclaw/openclaw/issues/66540) **S** replyToCurrent default false blocks implicit replyToId | 🔍 已覆盖(方向1 GitHub 第124轮 21:50 CST) | **已在 PR #66562 覆盖**，无需重复追踪 |
| P60110 | [#66534](https://github.com/openclaw/openclaw/issues/66534) **S** lifecycle:end event missing aborted/stopReason | 🔍 已覆盖(方向1 GitHub 第124轮 21:50 CST) | **已在 PR #66574 覆盖**，无需重复追踪 |
| P60111 | [#66549](https://github.com/openclaw/openclaw/issues/66549) **S** dry-run still delivers message | 🔍 已覆盖(方向1 GitHub 第124轮 21:50 CST) | **已在 PR #66559 覆盖**，无需重复追踪 |
| P60112 | [#66514](https://github.com/openclaw/openclaw/issues/66514) **S** Browser control reports healthy but CDP path dead | 🔍 已在 aoao 追踪(方向1 GitHub 第124轮 21:50 CST) | **已在 aoao 追踪**（原 P400 区） |
| P60113 | [Tencent/openclaw-weixin #65](https://github.com/Tencent/openclaw-weixin/issues/65) **S** 微信消息接收乱序，网页端能收到微信收不到 | 🔍 新发现(方向2 插件 第124轮 21:50 CST) | 近24小时内新 issue；可能与 openclaw core message ordering 相关；**建议跟进确认** |
| P60114 | [方向4 第124轮 21:50 CST] **PR #66575 maintainer 新评论（P1×2）：** dry-run 分支没有调用 runMessageAction 导致 result 缺少必需字段；aborted 逻辑误判 stop reason；**PR #66574 maintainer P2：** MediaPaths 和 MediaTypes 独立过滤可能导致 index 不对齐 | 🔍 **maintainer 新反馈**(方向4 第124轮 21:50 CST) | 需要 review 确认 fix 是否完整 |
| P60115 | [方向4 第124轮 21:50 CST] **PR #66578** (chore: package.json metadata) — 新开，0评论；**PR #66568** (fix streaming: prevent markdown table split) — 新开，0评论；**PR #66572** (fix: honor cfg-compatible capability provider resolution) — 新开，0评论 | 👀 状态变化(方向4 第124轮 21:50 CST) | 新 PR 需持续追踪 merge 时机 |
| P60116 | [方向4 第125轮 23:00 CST] **PR #66556** (fix telegram: filter undefined paths for voice transcription) — size XS by oraculoos, updated 14:52 UTC；**PR #66555** (fix subagent: suppress exec approval followups for all subagent sessions) — size XS by oraculoos, updated 14:52 UTC | 🔍 **Review 机会**(方向4 第125轮 23:00 CST) | 两个 XS PR 同时来自 oraculoos；test coverage 好；maintainer 尚未 review；**建议立即 review + approve**，cost 低收益高 |
| P60117 | [方向4 第125轮 23:00 CST] **PR #66552** (fix: recover reasoning-only Ollama turns) — size S by yfge；**PR #66548** (fix: strip inbound metadata envelopes from session corpus) — size S by zqchris；**PR #66547** (fix reply: keep implicit threading when replyToCurrent defaults false) — size XS by neeravmakwana | 👀 状态变化(方向4 第125轮 23:00 CST) | 多个外部 PR 需要 maintainer review；持续追踪 merge 时机 |
| P60118 | [方向1 第125轮 23:00 CST] **PR #66622** `fix(image): prepare dynamic models before image tool registry lookup` — by zhanggpcsu, size S, **刚创建** (2026-04-14T15:00:33Z) | 🔍 新发现(方向1 第125轮 23:00 CST) | 本轮扫描期间新创建；需持续追踪 review 进展 |
| P60095 | [Tencent/openclaw-weixin #50](https://github.com/Tencent/openclaw-weixin/issues/50) **S** openclaw 定时推送无法在微信触发 | 🔍 新发现(方向2 插件 第123轮 17:27 CST) | WeChat cron push 在无 inbound 消息时失效约40分钟；可能是长连接 idle timeout 机制问题；用户证据充分；**建议跟进 WeChat 插件 cron 投递路径** |
| P60084 | [#43767](https://github.com/openclaw/openclaw/issues/43767) **M regression** Heartbeat ignores lightContext: true，加载完整 context + unbounded history | 🔍 建议 review 后接单(方向1 GitHub 第122轮 15:19 CST) | heartbeat 忽略 lightContext，每次 tick 加载完整 context 导致 token 无界增长；4 条评论，author 有深入分析 |
| P60085 | [#66389](https://github.com/openclaw/openclaw/issues/66389) **M regression** acpx embedded runtime 每个消息 spawn 新进程（2026.4.11） | 🔍 建议 aoao 接单(方向1 GitHub 第122轮 15:19 CST) | acpx embedded runtime 每个消息都 spawn 新进程而非复用，cache hit rate 100%→35%，cost 激增 |
| P59910 | [#65211](https://github.com/openclaw/openclaw/issues/65211) **S** include user-configured provider models in gateway catalog | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | PR size S；根因：`loadGatewayModelCatalog` 未合并 `models.providers.*.models[]`；导致 `resolveGatewayModelSupportsImages` 永远返回 false，图片附件被静默丢弃；supersedes #65178；**建议 review 优先** |
| P59911 | [#65208](https://github.com/openclaw/openclaw/issues/65208) **S** Browser plugin 不传 `--remote-debugging-port` 导致 Chrome CDP 无法连接 | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | macOS 2026.4.11；config 里 `cdpPort: 18800` 正确但 launch command 里没有对应 flag；浏览器自动化功能完全不可用；**建议 aoao 接单** |
| P59912 | [#65207](https://github.com/openclaw/openclaw/issues/65207) **S** Ollama models not displayed in openclaw models list (2026.4.11) | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | 2026.4.10: hangs/gets killed; 2026.4.11: returns immediately "No models found"；直接 curl 正常，说明问题在 OpenClaw 侧 model registry 解析；**建议 aoao 接单** |
| P59913 | [#65204](https://github.com/openclaw/openclaw/issues/65204) **S** Cannot start Chrome, cdpPort binding Permission denied | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | `bind() failed: Permission denied (13)` on port 80；标签: bug+regression；可能与 #65208 同根但场景不同；**建议 aoao 接单** |
| P59914 | [#65209](https://github.com/openclaw/openclaw/issues/65209) **S** `openclaw mcp list` 只显示 OpenClaw registry，不包含 mcporter 配置 | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | docs gap + usability；用户期望看到系统全部 MCP servers，实际只看到 `~/.openclaw/openclaw.json` 里的；修复简单（S 级）；建议 aoao 接单（修 CLI wording + help text） |
| P59915 | [#65200](https://github.com/openclaw/openclaw/issues/65200) **S** /new 和 /reset 不清除 session model overrides | 🔍 新发现(方向1 GitHub 第99轮 14:51 CST) | `/model codex` 设置后，`/new` 仍保留 override，导致 `modelByChannel` 配置被绕过；根因：`initSessionState()` 在 resetTriggered 时复制了 `modelOverride`/`providerOverride`；建议 aoao 接单 |
...
| P59918 | [方向4 PR 监控 第109轮 06:36 CST] PR #65577 (OutboundPayloadPlan centralization, size:L) — maintainer, 2评论, refactor, 测试覆盖好；PR #65574 (MCP stdio proxy, size:M) — maintainer P1×2 已修复（75dbc98），新 P1（lowercase proxy precedence）需确认 75dbc98 是否覆盖；PR #65575 (UI aria-live streaming fix, size:XS) — draft 状态；PR #65511 (Gateway timeout attribution+QA restart, size:XL) — linked to #65504/505/506，已关闭 | 👀 状态变化(方向4 第109轮 06:36 CST) | 新增 #65577（size:L refactor，测试覆盖好）；持续追踪 #65574 最新 P1 覆盖状态；#65511 被 linked issues 关闭 |
| P59948 | [#65613](https://github.com/openclaw/openclaw/issues/65613) **(bug+regression)** Discord stale-socket health-monitor restart every ~35min | 🔍 新发现(方向1 GitHub 第113轮 00:45 UTC) | regression；与历史 #38395 同根问题，已持续多年；maintainer 刚刚更新该 issue，说明正在关注；**建议跟进 review 机会** |
| P59949 | [#65598](https://github.com/openclaw/openclaw/issues/65598) **(bug)** WhatsApp channel crashes on start — missing path/fs imports in login module (v2026.4.11) | 🔥 **aoao 已派出(runId e441d8de)**(方向1+方向3 第113轮 00:45 UTC) | **直接命中贡献者 mcaxtr 文件区**：mcaxtr 今日 commit 全在 `extensions/whatsapp/src/`（login-qr.ts, connection-controller.ts 等），恰好是 crash 根因所在 login 模块；crash 明确，v2026.4.11 regression，0评论未认领；**runId e441d8de** |
| P59950 | [#65611](https://github.com/openclaw/openclaw/issues/65611) **(bug+regression)** manual-cdp attachOnly profile not detecting active CDP session | 🔍 新发现(方向1 GitHub 第113轮 00:45 UTC) | regression；manual-cdp profile 功能失效，无法检测已有 CDP session；建议跟进 |
| P59951 | [Tencent/openclaw-weixin PR #56](https://github.com/Tencent/openclaw-weixin/pull/56) **feat(bindings): add ACP thread binding adapter for WeChat** | 🔍 新发现(方向2 插件 第113轮 00:45 UTC) | WeChat 支持 ACP thread binding 使 session mode 可用；完整 PR 已 open；**建议跟进 review，确认是否与 openclaw core 有交互** |
| P59952 | [#65623](https://github.com/openclaw/openclaw/issues/65623) **🔒 S** /config show returns unredacted config with plaintext secrets | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | 安全 bug；配置显示 API 返回明文 secrets，secret redaction 失效；**建议 aoao 优先接单** |
| P59953 | [#65625](https://github.com/openclaw/openclaw/issues/65625) **🔒 S** openclaw.podman.env ships empty gateway token with lan bind as default | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | 安全 gap；podman env 默认空 token + lan bind，直接暴露未授权 gateway；**建议 security 高优先级** |
| P59954 | [#65624](https://github.com/openclaw/openclaw/issues/65624) **🔒 S** Mattermost slash commands default to cleartext callback URLs exposing reusable command tokens | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | 安全 bug；Mattermost 斜杠命令使用明文 callback URLs，暴露可重放 command tokens；**建议 security 高优先级** |
| P59955 | [#65646](https://github.com/openclaw/openclaw/issues/65646) **M** Fallback pollutes session runtime model, causing silent model drift | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | fallback chain 触发后 session runtime model 被静默污染，导致 model drift；0评论无标签未认领；可能与 #65234/#65086 同簇；**建议 review 后考虑接单** |
| P59956 | [#65619](https://github.com/openclaw/openclaw/issues/65619) **S regression** macOS: gateway bind=loopback resolves to 0.0.0.0 and refuses to start (2026.4.11) | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | macOS 上 loopback 绑定被解析为 0.0.0.0，health check 报 1006；可能与 #65208/#65204/#65571 同根；**建议 aoao 接单** |
| P59957 | [#65643](https://github.com/openclaw/openclaw/issues/65643) **S** --profile flag does not clear OPENCLAW_LAUNCHD_LABEL, causing gateway status to resolve wrong plist | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | --profile 不清除 LAUNCHD_LABEL，导致 gateway status 解析到错误的 plist；S 级 fix；**建议 aoao 接单** |
| P59958 | [#65650](https://github.com/openclaw/openclaw/issues/65650) **feat** wire SQLite message store into active gateway for SIGTERM resilience | 🔍 新发现(方向1 GitHub 第114轮 09:54 CST) | 新功能请求；SQLite message store 做 SIGTERM resilience；0评论无标签；Feature 级别，低优先级 |
| P59959 | [Tencent/openclaw-weixin #57](https://github.com/Tencent/openclaw-weixin/issues/57) **S** unable to uninstall plugin after npx install | 🔍 新发现(方向2 插件 第114轮 09:54 CST) | 用户无法卸载 weixin 插件；Error: Cannot find module 'zod' + duplicate plugin id；**建议确认是否与插件 staging 路径问题相关** |
| P59960 | [Tencent/openclaw-weixin PR #49](https://github.com/Tencent/openclaw-weixin/pull/49) **S** fix: preserve quote context (ref_msg) for voice messages | 🔍 新发现(方向2 插件 第114轮 09:54 CST) | VOICE 消息的 ref_msg（引用上下文）处理缺失，与 TEXT 分支不一致；0评论可做 S 级 fix；**建议跟进** |
| P59961 | [方向4 第114轮 09:54 CST] PR #65574 MCP stdio proxy 连续修复：KeWang0622 在 75dbc98 → 999eda9 连续 5 个 fix 提交，修复 proxy precedence、NODE_OPTIONS stripping 等问题；chatgpt-codex-connector[bot] P1×2 均已修复 | 🔍 **Author 连续修复中**(方向4 第114轮 09:54 CST) | **持续追踪 merge 时机**；author 响应积极；#65574 size:M 接近可 merge |
| P59962 | [方向4 第114轮 09:54 CST] PR #65511 Gateway timeout attribution+QA restart：Copilot C1 type assertion 问题；chatgpt-codex-connector[bot] P1 waitForConfigRestartSettle 竞态（100yenadmin 确认 VALID P1，2f1e865f50 修复） | 👀 状态变化(方向4 第114轮 09:54 CST) | 持续 review 中；branch 已有 2f1e865f50 修复 waitForConfigRestartSettle 竞态；关注后续 review 循环 |
...
## 🔴 xixi 第51轮扫描（2026-04-13 06:36 CST / 22:36 UTC）

**新发现高优先级候选**：
- **#65576**（S级 regression）：v2026.4.11 cron 默认禁用 LLM idle watchdog，主 provider hung 时整个 cron timeout 被消耗，failover chain 无法启动。建议接单 fix
- **#65572**（M级）：TUI chat.history 在 gateway restart 后因 readiness race 失败，可提 PR 修复
- **#65574**（M级）：MCP stdio proxy env，maintainer bot 提了 2 个 P1，author 已 reply 修复75dbc98；新 P1（lowercase proxy precedence，22:38）需确认是否已覆盖

**无**：openclaw-weixin 仓库 404（不存在）；底部10位贡献者 commit 文件列表全空（长期不活跃）
...
### 方向1 新候选 P731-P740（2026-04-12 14:51 CST）
- **P731** [#65210](https://github.com/openclaw/openclaw/issues/65210) **P1 + regression** Control UI token usage shows N/A, usage page missing token counts, and auto-compression fails after 2026.4.9 | 三重症状：(1) Sessions Tokens 列显示 "unknown/262k (?%)"; (2) Usage 页面 token 统计缺失; (3) auto-compression 失效 → context overflow; 根因: OpenClaw 无法解析 API 响应的 `usage` 字段; 影响所有 2026.4.9+ 用户; **最高优先级，aoao 优先接单**
- **P732** [#65211](https://github.com/openclaw/openclaw/pull/65211) **S** fix: include user-configured provider models in gateway catalog (supersedes #65178) | 根因修复：`models.providers.*.models[]` 中 custom model 完全不在 Pi SDK registry 里；导致 `resolveGatewayModelSupportsImages` 永远 false，图片附件被静默丢弃；比 #65178 的 case-insensitive lookup 更彻底; 1条评论; **建议 aoao 优先 review**
- **P733** [#65208](https://github.com/openclaw/openclaw/issues/65208) **S** Browser plugin 不传 --remote-debugging-port 导致 Chrome CDP 无法连接 (macOS) | `openclaw browser start` 启动 Chrome 但 launch command 缺少 `--remote-debugging-port` flag；config `cdpPort: 18800` 正确但未使用；浏览器自动化完全不可用; **建议 aoao 接单（S 级）**
- **P734** [#65207](https://github.com/openclaw/openclaw/issues/65207) **S** Ollama models not displayed in openclaw models list (2026.4.11) | direct curl `http://127.0.0.1:11434/api/tags` 正常返回；说明根因在 OpenClaw 侧 model registry 解析; 2026.4.10: hangs; 2026.4.11: "No models found"; **建议 aoao 接单**
- **P735** [#65204](https://github.com/openclaw/openclaw/issues/65204) **S + regression** Cannot start Chrome — cdpPort binding Permission denied | macOS 2026.4.10；`bind() failed: Permission denied (13)` on port 80；regression 标签；注意与 #65208 可能同根（不同 port/场景）
- **P736** [#65209](https://github.com/openclaw/openclaw/issues/65209) **S + docs gap** `openclaw mcp list` 只显示 OpenClaw registry，不包含 mcporter 配置 | UX 缺口；用户期望看到系统全部 MCP servers，实际只看到 `~/.openclaw/openclaw.json`；建议 CLI wording 改进 + help text 说明; **文档/UX 缺口，S 级**
- **P737** [#65200](https://github.com/openclaw/openclaw/issues/65200) **S** /new 和 /reset 不清除 session model overrides | `/model codex` 设置后 `/new` 仍保留 override；根因: `initSessionState()` 在 resetTriggered 时复制了 modelOverride/providerOverride; **建议 aoao 接单**
- **P738** [#65195](https://github.com/openclaw/openclaw/issues/65195) **M** exec/runtime 无法访问 LAN host，interactive shell 正常 (macOS) | launchd/service 路径与交互 shell 网络视图隔离；"No route to host"；影响所有依赖 LAN 服务的 exec 工具; **建议 aoao 调研网络隔离根因**
- **P739** [#65192](https://github.com/openclaw/openclaw/issues/65192) **S** too many dream sessions accumulation — sessions 面板暴露不应显示给用户的内容 | bug+bug:behavior；截图显示大量 dream session 堆积；dream session 应有 maxAge 或自动清理机制; **建议 aoao 接单**
- **P740** [#65213](https://github.com/openclaw/openclaw/pull/65213) **S** feat(sessions): add effective session explain command | CLI 新功能；`openclaw sessions explain <key>` 调试 session 解析路径；size S

### 方向4 PR 新动态（2026-04-12 14:51 CST）
- **#65205** feat(discord): add canvas-first Discord Activities support | **maintainer XL PR，持续活跃**；新评论来自 greptile P1×2 / P2×2 + chatgpt-codex-connector P1×2；关键问题：(1) `hasDiscordActivityLaunchContext` 需 reject spoofed launch markers 才给 auth bypass; (2) `OpenClaw.discord.load()` 失败后未 reset promise 导致无法 retry; (3) "Every N hours" 描述静默丢弃 minute offset; (4) `formatPhaseSchedule` 缺少 next-run fallback 时显示 "—"; **建议在 merge 前解决 P1 问题**
- **#65202** fix(feishu): add doctor migration for legacy botName→name | **greptile P2×2**；(1) migration log message 当 `name` 已存在时仍 push 误导性变化; (2) `describe` 遗漏 channel-level case; **maintainer 评论**: 不应 rename top-level `channels.feishu.botName`，应该是 per-account 层
- **#65194** fix(imessage): avoid duplicate default monitor startup | **maintainer P1×2 / P2×2**；(1) `hasDistinctDefaultIMessageRuntime` 检查范围过宽; (2) `skipDefaultFallbackMonitor` 未 ignore disabled accounts; author 已更新
- **#65199** fix(ui): humanize cron expressions in dreaming phase | 多个 bot 评论（P2: minute offset 被静默丢弃、next-run fallback 缺失）；author 已在处理

## xixi 第95轮扫描（2026-04-12 14:51 CST）

### 执行状态
- **方向1 GitHub Issues**：扫描过去 ~2 小时 open issues/PRs（04:51-06:51 UTC）。发现 11 个新 issue + 13 个新 PR。最重要: **#65210** (token usage N/A regression)、**#65211** (provider models catalog 根因修复 supersedes #65178)、**#65208** (browser --remote-debugging-port missing)、**#65207** (Ollama models not listed)、**#65200** (/new 不清除 model override)、**#65195** (LAN host unreachable from exec/runtime, macOS)
- **方向2 插件仓库**：`Tencent/openclaw-weixin` 和 `openclaw/openclaw-weixin` 均返回 404 Not Found；**无公开可访问的插件 issue/PR**。本轮无新发现
...
| P741 | [#65341](https://github.com/openclaw/openclaw/issues/65341) **bug:behavior** memory-core dreaming narrative 每次运行静默失败 — `idempotencyKey` 必填字段被条件 spread 省略 | 🔥 **aoao 已派出**(方向1 GitHub 第101轮 20:55 CST) | 根因已定位：`server.impl-*.js` 的 `createGatewaySubagentRuntime().run()` 用 `...params.idempotencyKey && {...}` 可省略必填字段；`AgentParamsSchema` 要求非空；次生问题：cron 仍报 `lastRunStatus: "ok"`；**S 级，workaround 已给出，1-2 小时可 PR** |
| P742 | [#65347](https://github.com/openclaw/openclaw/issues/65347) **bug+regression** OpenAI Codex OAuth `invalid_scope` — `model.request` scope 被 OpenAI 拒绝，v2026.4.10 起认证完全失败 | 🔥 **aoao 已派出**(方向1 GitHub 第101轮 20:55 CST) | OAuth flow 报 `invalid_scope` 和 `missing authorization code`；**调研 OAuth scope 变更是否 OpenAI API 层面** |
...
| P59966 | [#65498](https://github.com/openclaw/openclaw/issues/65498) **bug:crash** Main-session user task loses final reply after heartbeat/exec-completion interrupt | 🔥 **aoao 已派出**(方向1 第105轮 03:37 CST) | bug:crash；心跳或 exec 完成中断时，原始任务的最终用户回复丢失；serious；**runId 805402eb** |
...
| P59972 | [#65529](https://github.com/openclaw/openclaw/issues/65529) **[Bug][regression]** Microsoft Edge TTS provider not registered — `microsoft: no provider registered` | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | bug+regression；2026.4.11；模块完全没加载；其他 provider 有错误但 Edge 静默失败；0评论，未认领；**建议 xixi 调研 TTS 注册链路根因** |
| P59973 | [#65528](https://github.com/openclaw/openclaw/issues/65528) **[Bug]** Managed browser process dies silently 1-2s after spawn on macOS (attachOnly ignored) | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 极详细报告；根因假设：gateway spawn mechanism 主动 kill Chrome child；手动启动正常；`attachOnly: true` 和 `existing-session` 完全被忽略；与 #65521 同类；0评论；**建议 xixi 调研 spawn lifecycle** |
| P59974 | [#65524](https://github.com/openclaw/openclaw/issues/65524) **[Bug][security]** Deleted agent's sessions remain accessible and executable via sessions_send | 🔥 **aoao 已派出**(方向1 GitHub 第105轮 03:37 CST) | 高安全风险；删除 agent 后 .jsonl/sessions.json 仍可访问，`sessions_send` 继续执行特权操作（exec/write/gateway config）；0评论，未认领；**runId f3897832** |
| P59975 | [#65517](https://github.com/openclaw/openclaw/issues/65517) **[Bug]** Active-memory embedded sub-agent blocks event loop — Telegram permanently unresponsive | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | Critical；100% reproducible；event-loop 饿死 Telegram getUpdates；与 #56733/#52231/#9751 同簇；0评论，未认领 |
| P59976 | [#65521](https://github.com/openclaw/openclaw/issues/65521) **[Bug][regression]** Windows managed browser — Chrome starts then immediately exits, CDP never binds | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 与 #65528 同类；Windows 11；手动 PowerShell 正常；gateway spawn 后立即退出；0评论；**建议与 #65528 联合调研** |
| P59977 | [#65514](https://github.com/openclaw/openclaw/issues/65514) **[Bug]** MiniMax tokenBudget not forwarded → LCM compaction silently skipped | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | High；MiniMax 所有长 session 最终 context overflow crash；error 2013 被写入 messages 表污染后续 session；0评论，未认领 |
| P59978 | [#65520](https://github.com/openclaw/openclaw/issues/65520) **[Bug]** Main session stdio MCP transport fails on Windows — schemas load but calls return "Not connected" | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 静默失败最坏情况；subprocess 测试 MCP 正常，问题在 gateway StdioClientTransport；0评论，未认领 |
| P59979 | [#65519](https://github.com/openclaw/openclaw/issues/65519) **[Bug]** sessions_spawn model override silently ignored — sub-agents always use primary model | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | model ID rewrite layer 问题；返回 `modelApplied: true` 但静默忽略；与 #6295/#57428/#51545 同簇；0评论，未认领 |
| P59980 | [#65525/#65526](https://github.com/openclaw/openclaw/issues/65525) MCP stdio 不继承 NODE_OPTIONS/proxy env vars | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 同一作者两个 issue（feature+bug）；`resolveStdioMcpServerLaunchConfig` 未合并 gateway proxy 状态；1评论；建议合并为同一 PR |
| P59981 | [#65513](https://github.com/openclaw/openclaw/issues/65513) **Bug** github-copilot extension crashes agent on every message (ERR_MODULE_NOT_FOUND) | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 所有 channel 全面崩溃；bundled extension 导入路径 `openclaw/plugin-sdk/provider-auth` 损坏；0评论，未认领 |
| P59982 | [#65522](https://github.com/openclaw/openclaw/issues/65522) Control UI refreshing chat data during active turn wedges page/gateway | 🔍 新发现(方向1 GitHub 第102轮 03:37 CST) | 100yenadmin 报告；直接 UI footgun；0评论 |
| P59983 | [方向4 PR #65527 maintainer review] fix(speech): allow media-only tagged TTS replies — maintainer 新增 P1×1/P2×2 inline comments（19:25-19:27 UTC）| 👀 maintainer 新评论(方向4 第102轮 03:37 CST) | P1 要求 `allowText` policy 对 `[[tts:text]]` 保留 enforcement；P2 指出 `synthesizeTelephony` 缺少 NonNullable assertion + dead code；author 未回复 |
| P59984 | [方向4 PR #65512 maintainer review] fix: infer WhatsApp group reaction participant — maintainer P2 comment（19:03 UTC）| 👀 maintainer 新评论(方向4 第102轮 03:37 CST) | `inferredParticipant` 应限于 context 派生的 message ID；author 未回复 |
| P59985 | [方向4 PR #65511 maintainer review] Stabilize live gateway timeout attribution（XL）— maintainer P1×1/P2×2 + greptile comments（19:15-19:23 UTC）| 👀 maintainer 新评论(方向4 第102轮 03:37 CST) | `waitForConfigRestartSettle` 过早返回；IPv6 loopback 检查可能遗漏；author 未回复 |
| P59986 | [方向2 Tencent/openclaw-weixin #57] unable to uninstall | 🔍 新发现(方向2 插件 第102轮 03:37 CST) | updated 2026-04-12 09:18 UTC；用户卸载路径问题；代码不可见 |
| P59987 | [#65548](https://github.com/openclaw/openclaw/issues/65548) **Bug: Telegram/Discord incoming message processing crash after commit 74f31241ed** | 🔥 **最高优先级**(方向1 GitHub 第108轮 05:31 CST) | **无标签，0评论**；commit 74f31241ed（fix(runtime): hide lazy command and context seams）引入两个阻塞 bug；**100% 消息投递失败**——Telegram/Discord 所有入站消息不响应，Gateway 变无响应；精确 commit SHA，根因明确；影响所有 Telegram/Discord 用户，**立即阻断**，S/M 级可修性；建议 aoao 优先接单 |
| P59988 | [#65550](https://github.com/openclaw/openclaw/issues/65550) **[Bug]: memory-core dreaming runaway loop — 94 sessions spawned in 65 min, $4.35 burned on zero-confidence garbage** | 🔥 **高优先级**(方向1 GitHub 第108轮 05:31 CST) | **无标签，0评论**；dreaming 功能失控：65分钟内生成94个子会话，$4.35被烧掉；零置信度垃圾无限生成，**直接烧钱**；P1 优先级；建议 aoao 接单 |
| P59989 | [#65517](https://github.com/openclaw/openclaw/issues/65517) **[Bug]: Active-memory embedded sub-agent run blocks event loop, starving Telegram polling** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | bug 标签，0评论；活跃 memory sub-agent 阻塞事件循环，导致 Telegram polling 饿死，Agent 永久无响应；与 #65548 同簇（Telegram 不可用），建议一起排查 |
| P59990 | [#65553](https://github.com/openclaw/openclaw/issues/65553) **[Bug]: openclaw memory CLI hangs with QMD backend on Raspberry Pi / arm64** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | bug+regression 标签；QMD backend 在 arm64/Raspberry Pi 上 CLI hang；0评论；建议 aoao 接单 |
| P59991 | [#65524](https://github.com/openclaw/openclaw/issues/65524) **[Bug]: Deleted agent's sessions remain accessible and executable** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | bug+bug:behavior 标签；删除的 agent 的 session 仍可访问和执行；安全+行为问题；建议 aoao 接单 |
| P59992 | [#65558](https://github.com/openclaw/openclaw/issues/65558) **[Bug]: onboard exposes minimax refs after MiniMax Portal OAuth setup** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签；onboard 流程暴露 minimax refs；安全/隐私缺口；0评论；建议 aoao 接单 |
| P59993 | [#65559](https://github.com/openclaw/openclaw/issues/65559) **[Bug]: allowBots: "mentions" silently drops webhook messages — mentionedUsers empty** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | bug 标签；allowBots mentions 模式静默丢弃 webhook 消息；0评论；建议 aoao 接单 |
| P59994 | [#65561](https://github.com/openclaw/openclaw/pull/65561) **fix(runtime): add missing tsdown entries for lazy runtime modules** | 🔍 新发现(方向4 PR 第108轮 05:31 CST) | garnetlyx:fix/telegram-discord-crash-runtime-imports；+2/-0；greptile-apps 已背书；size XS；修复 Telegram/Discord crash runtime imports；**建议 review 优先** |
| P59995 | [方向3 贡献者文件区 第108轮 05:31 CST] 扫描了排名最末10个贡献者（quotentiroler/eleqtrizit/altaywtf/Glucksberg/onutc/solmaz/jalehman/mcaxtr/bmendonca3/huntharo，贡献量45~70）| 📋 无新发现 | 200条 commit 均未附带可见 files diff 信息（API返回files=[]），可能是这些用户只参与 docs/wiki/配置文件；**未从文件区发现新的未认领高优先级 open bug** |
| P59996 | [方向4 PR #65511 maintainer review] Stabilize live gateway timeout attribution（XL）— maintainer @steipete @vincentkoc 追加评论要求更好测试和日志（21:08 UTC）| 👀 状态变化(方向4 第108轮 05:31 CST) | 100yenadmin 的 PR；greptile 也已 COMMENTED；正向状态，需 author 回复 maintainer 问题 |
| P59997 | [#65528](https://github.com/openclaw/openclaw/issues/65528) **[Bug] Managed browser process dies silently 1-2s after spawn on macOS (attachOnly ignored)** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；有用户深度分析；根因假设：gateway spawn mechanism 主动 kill Chrome child；手动启动正常；`attachOnly: true` 和 `existing-session` 完全被忽略；**建议 aoao 调研 spawn lifecycle** |
| P59998 | [#65560](https://github.com/openclaw/openclaw/issues/65560) **Surface Chrome MCP existing-session failures explicitly** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；Chrome MCP existing-session 失败时应明确报错而非静默；建议 aoao 接单 |
| P59999 | [#65557](https://github.com/openclaw/openclaw/issues/65557) **Provider & model selection per session/account with admin-controlled allowlists** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；功能请求；per-session/account 的 provider/model 选择配合 admin allowlist；建议评估 |
| P60000 | [#65556](https://github.com/openclaw/openclaw/issues/65556) **LLM Dispatcher: native GitHub Copilot provider** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；功能请求；原生 GitHub Copilot provider；建议评估 |
| P60001 | [#65551](https://github.com/openclaw/openclaw/issues/65551) **Local MLX/LM Studio models get "terminated" on RAM pressure** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；本地 MLX/LM Studio 模型在内存压力下被终止；建议 aoao 接单 |
| P60002 | [#65544](https://github.com/openclaw/openclaw/issues/65544) **agent_end hook has no timeout: hung embedding endpoint silently drops memory extraction** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；agent_end hook 无超时，embedding endpoint 卡死时 memory extraction 被静默丢弃；建议 aoao 接单 |
| P60003 | [#65538](https://github.com/openclaw/openclaw/issues/65538) **Screen readers announce every token during streaming (Accessibility)** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；Accessibility bug；屏幕阅读器在 streaming 时逐 token 播报；建议 aoao 接单 |
| P60004 | [#65533](https://github.com/openclaw/openclaw/issues/65533) **reasoning_details not preserved in MiniMax message history** | 🔍 新发现(方向1 GitHub 第108轮 05:31 CST) | 无标签，0评论；MiniMax reasoning_details 未保留在消息历史中；建议 aoao 接单 |
| P60005 | [#65584](https://github.com/openclaw/openclaw/issues/65584) **S** cron CLI commands succeed but hang instead of exiting cleanly | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | bug；cron store mutation 成功但 CLI hang；CLI/gateway transport teardown bug；0评论；**建议 aoao 接单（S 级）** |
| P60006 | [#65578](https://github.com/openclaw/openclaw/issues/65578) **regression** Kilo Gateway model auto-discovery only returning Kilo/kilo-auto | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | regression+bug；只有1个模型可选而非500+；2026.4.11；1评论；**建议 aoao 接单** |
| P60007 | [#65581](https://github.com/openclaw/openclaw/issues/65581) **bug:behavior** Agent emits duplicate Discord messages on every response | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | streaming关闭仍双发；0评论；**建议 aoao 接单** |
| P60008 | [#65576](https://github.com/openclaw/openclaw/issues/65576) **M** Cron runs silently disable LLM idle watchdog by default (v2026.4.11) | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | cron无explicit idleTimeout时 watchdog 被禁用，providers 耗尽；0评论；**建议 aoao 接单** |
| P60009 | [#65594](https://github.com/openclaw/openclaw/issues/65594) **M** packaging: runtime dist build does not emit control-ui/ when only pnpm build is run | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | packaging bug；build 流程不完整；最新更新 23:39 UTC；**建议 aoao 接单** |
| P60010 | [#65590](https://github.com/openclaw/openclaw/issues/65590) **S** MCP streamable-http transport not forwarding custom headers to remote servers | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | MCP bug；0评论；**建议 aoao 接单** |
| P60011 | [#65572](https://github.com/openclaw/openclaw/issues/65572) **M** TUI chat.history fails without retry on gateway restart race condition | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | bug；gateway restart 后 chat subsystem 未 ready 时 chat.history 被拒绝；0评论；**建议 aoao 接单** |
| P60012 | [#65568](https://github.com/openclaw/openclaw/issues/65568) **S** Discord-bound persistent Codex ACP session cannot be resumed | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | bug；ACP session 恢复失败；0评论 |
| P60013 | [#65566](https://github.com/openclaw/openclaw/issues/65566) **S** Streaming partialParse errors crash agent runs | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | JSON 解析错误导致 crash；0评论 |
| P60014 | [#65563](https://github.com/openclaw/openclaw/issues/65563) **S** plugins/webhooks create_flow fails with NOT NULL constraint | 🔍 新发现(方向1 GitHub 第106轮 07:39 CST) | DB schema 问题；0评论 |
| P60015 | [方向4 第106轮 07:39 CST] PR [#65589](https://github.com/openclaw/openclaw/pull/65589) `feat(memory-core): dreaming circuit breaker` — maintainer chefallan P1 review：DreamingBudgetEnforcer 从未被接入 dreaming pipeline（`dreaming.ts` 和 `dreaming-p*` 未调用 Enforcer）；⚠️ PR 修 budget enforcer 但未连接调用路径，修了个寂寞 | 🔥 **aoao 需确认覆盖范围**(方向4 第106轮 07:39 CST) |
| P60016 | [方向4 第106轮 07:39 CST] PR [#65577](https://github.com/openclaw/openclaw/pull/65577) `Outbound: centralize payload normalization` — aisle security bot 报告高危安全问题：MEDIA directive 可被恶意利用读取本地文件（`MEDIA:/etc/passwd`）；PR 测试覆盖好（greptile 5/5）但安全缺陷未处理；merge 前需确认安全修复 | ⚠️ **merge 前需确认安全修复**(方向4 第106轮 07:39 CST) |
| P60017 | [方向2 Tencent/openclaw-weixin #57] unable to uninstall | 🔍 新发现(方向2 插件 第106轮 07:39 CST) | updated 2026-04-12 09:18 UTC；超出2小时窗口；代码不可见 |
| P60018 | [方向3 贡献者文件区 第106轮 07:39 CST] lml2468 最近10个commit涉及 gateway/server-maintenance/nodes/memory-flush 等核心路径；相关 open bugs（#64911/#64984/#55679）均已追踪；无新的未认领高优先级项 | 📋 无新发现(方向3 第106轮 07:39 CST) |

| P60019 | [#65676](https://github.com/openclaw/openclaw/issues/65676) **Bug + Regression** Model fallback never actually executes — live session model switch overrides fallback in loop | 🔥 **建议派 aoao 接单**(方向1 GitHub 第115轮 11:09 CST) | **最高优先级 regression**；根因：auth-profiles-*.js 中 `resolvePersistedLiveSelection()` 在每次循环迭代把 fallback 选中的模型强制覆盖回 primary，导致无限循环直到 retry limit；logs 清晰展示了 fallback → model switch → throw → 重置回 primary → 再 fallback 的循环；影响所有配置了 fallback 模型的用户；**建议立即派 aoao** |
| P60020 | [#65660](https://github.com/openclaw/openclaw/issues/65660) **Bug (HIGH)** TTS audio base64 blobs inflate context token estimate → compaction death spiral | 🔥 **建议派 aoao 接单**(方向1 GitHub 第115轮 11:09 CST) | 22 条 TTS 消息 = 11.2MB（占 session 文件 12.3MB 的 90%）；`estimateContentBlockChars()` 把音频 blob 当普通 unknown 类型序列化，估计为 ~250K tokens；compaction 无法总结音频 → 失败 3 次 → session 重启历史全丢；修复：加 `isAudioBlock` 检查返回小常数而非 blob 长度；**建议派 aoao（两行 patch）** |
| P60021 | [#65659](https://github.com/openclaw/openclaw/issues/65659) **Bug (HIGH)** sessions.json entries missing sessionId and sessionFile — thread context lost on restart | 🔥 **建议派 aoao 接单**(方向1 GitHub 第115轮 11:09 CST) | 76% sessions.json 条目缺少 sessionId/sessionFile；根因：store 写路径创建了 key entry 但从未在 embedded agent run 完成后回填这两个字段；JSONL 文件存在但sessions.json 从不链接；修复后 Slack/Discord thread 对话历史在 restart 后可恢复；**建议派 aoao** |
| P60022 | [#65667](https://github.com/openclaw/openclaw/issues/65667) **Bug (HIGH)** browser: CDP websocket health check never attempts TCP connect after /json/version returns 200 | 🔥 **建议派 aoao 接单**(方向1 GitHub 第115轮 11:09 CST) | Linux/Kali 特有；HTTP GET /json/version 返回 200，但 `waitForCdpReadyAfterLaunch` → `isReachable` → `isChromeCdpReady` → `canRunCdpHealthCommand` 链路在 8s 窗口内零 TCP connect；standalone node script 正常，gateway process 内部路径特有；疑似 gateway dispatcher 或 policy hook 拦截了 WebSocket 构造；**建议派 aoao** |
| P60023 | [#65668](https://github.com/openclaw/openclaw/issues/65668) **Bug + Regression** SIGUSR1/config.patch restart orphans gateway → EADDRINUSE crash loop | 🔍 新发现(方向1 GitHub 第115轮 11:09 CST) | 2026.4.11 regression；SIGUSR1 或 `config.patch` 触发重启后旧 gateway 进程被孤儿化，新进程尝试绑定同一端口 → EADDRINUSE crash loop；workaround: `OPENCLAW_NO_RESPAWN=1`；影响所有使用 config.patch 或 SIGUSR1 重启的用户；**建议 aoao 接单** |
| P60024 | [#65663](https://github.com/openclaw/openclaw/issues/65663) **Bug + Regression** Azure OpenAI Responses API — reasoning → tool_call history causes 400 on gpt-5.3-chat | 🔍 新发现(方向1 GitHub 第115轮 11:09 CST) | 2026.4.2 replay hook fix 未覆盖此场景；`reasoning_details` + `tool_call` 同时出现在 history 中导致 API 报 400 "required following item"；影响 Azure OpenAI + GPT-5.3-chat 用户；**建议 aoao 接单** |
| P60025 | [#65673](https://github.com/openclaw/openclaw/pull/65673) **fix(line): allow rich messages to use reply token** | 🔍 新发现(方向1 GitHub 第115轮 11:09 CST) | PR (size:XS)；fix #65656（LINE flex 消息因 Push API quota 限制被静默丢弃 429）；2 个参数 `false → true`，scope 极小；**建议快速 review + approve** |
| P60026 | [方向4 第115轮 11:09 CST] PR [#65574](https://github.com/openclaw/openclaw/pull/65574) MCP stdio proxy — author `chatgpt-codex-connector[bot]` 连续修复中，5 个 fix commits（75dbc98 → 999eda9），最新 P1（lowercase proxy precedence）需确认 75dbc98 是否已覆盖；**接近可 merge** | 🔍 **Author 连续修复中**(方向4 第115轮 11:09 CST) | **持续追踪 merge 时机** |
| P60027 | [方向4 第115轮 11:09 CST] 老 PR [#44815](https://github.com/openclaw/openclaw/pull/44815) Feishu: return [Image] placeholder for image messages — `lsdcc01` 突然活跃更新（03:12 UTC），最新活动；可能正在处理 Feishu 图片 MediaPath mismatch 问题 | 👀 **老 PR 突然活跃**(方向4 第115轮 11:09 CST) | 需确认是否需要 upstream openclaw API 改动；建议跟进 |

### 当前优先级更新（xixi 第115轮 2026-04-13 11:09 CST）

| P# | Issue | 优先级 | 备注 |
|----|-------|--------|------|
| P60019 | [#65676](https://github.com/openclaw/openclaw/issues/65676) **Bug + Regression** Model fallback never actually executes — live session model switch overrides fallback in loop | 🔥 最高 regression | `resolvePersistedLiveSelection()` 每次迭代强制覆盖 fallback；无限循环直到 retry limit；**aoao 已派出** |
| P60020 | [#65660](https://github.com/openclaw/openclaw/issues/65660) **Bug (HIGH)** TTS audio base64 blobs inflate context token estimate → compaction death spiral | 🔥 高 | 22条TTS消息=11.2MB；`estimateContentBlockChars()`序列化blob估计250K tokens；compaction失败3次→session重启；**aoao 已派出（两行patch）** |
| P60021 | [#65659](https://github.com/openclaw/openclaw/issues/65659) **Bug (HIGH)** sessions.json entries missing sessionId and sessionFile — thread context lost on restart | 🔥 高 | 76%sessions.json条目缺字段；store写路径未回填；修复后thread历史可恢复；**aoao 已派出** |
| P60022 | [#65667](https://github.com/openclaw/openclaw/issues/65667) **Bug (HIGH)** browser: CDP websocket health check never attempts TCP connect after /json/version returns 200 | 🔥 高 | Linux/Kali特有；HTTP 200但8s窗口零TCP connect；**aoao 已派出** |
| P60023 | [#65668](https://github.com/openclaw/openclaw/issues/65668) **Bug + Regression** SIGUSR1/config.patch restart orphans gateway → EADDRINUSE crash loop | 🔍 S | 2026.4.11 regression；workaround: `OPENCLAW_NO_RESPAWN=1`；**建议 aoao 接单** |
| P60024 | [#65663](https://github.com/openclaw/openclaw/issues/65663) **Bug + Regression** Azure OpenAI reasoning+tool_call history causes 400 on gpt-5.3-chat | 🔍 S | 2026.4.2 replay hook未覆盖此场景；**建议 aoao 接单** |
| P60025 | [PR #65673](https://github.com/openclaw/openclaw/pull/65673) **fix(line): allow rich messages to use reply token** | 🔍 XS | 2参数`false→true`，scope极小；**建议快速 review + approve** |

### aoao 派单状态（2026-04-13 11:15 CST）

| issue | 状态 | 备注 |
|-------|------|-------|
| #65676 Model fallback regression | 🔥 aoao 已派出 | P60019，最 high regression |
| #65660 TTS compaction death spiral | 🔥 aoao 已派出 | P60020，两行 patch |
| #65659 sessions.json 缺字段 | 🔥 aoao 已派出 | P60021，修复后 thread 历史可恢复 |
| #65667 CDP WebSocket health check | 🔥 aoao 已派出 | P60022，Linux/Kali 特有 |

| P60023 | [#65701](https://github.com/openclaw/openclaw/issues/65701) **(bug+regression)** Telegram provider sends duplicate messages for every response | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | regression；v2026.4.5 起每条回复发两次；0评论未认领；影响所有 Telegram 用户；S 级可本地复现 |
| P60024 | [#65698](https://github.com/openclaw/openclaw/issues/65698) **(bug:behavior)** memory-wiki bridge import reports 0 exported artifacts — memoryPluginState.capability not restored on cached plugin registry loads | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | 作者给出精确根因（plugin loader cache 不 restore `memoryPluginState.capability`）；已含修复建议；M 级，破坏 QMD+memory-wiki 混合工作流 |
| P60025 | [#65695](https://github.com/openclaw/openclaw/issues/65695) **(bug+regression)** Chrome CDP "not reachable after start" — gateway cannot connect to its own child Chrome process | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | macOS 2026.4.10；Chrome 能启动但 gateway HTTP health check 连不上 localhost:18800；作者已定位 `setGlobalDispatcher(new EnvHttpProxyAgent())` 干扰；L 级；命中 eleqtrizit 文件区；**建议跟进 review 机会** |
| P60026 | [#65676](https://github.com/openclaw/openclaw/issues/65676) **(bug:behavior)** Model fallback never actually executes — live session model switch overrides fallback in a loop | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | 作者给出完整根因分析（`resolvePersistedLiveSelection()` 把 fallback 当 user-initiated switch）；M 级；影响所有有 fallback 配置的用户 |
| P60027 | [#64793](https://github.com/openclaw/openclaw/issues/64793) **(bug:behavior)** Agent timeout does not surface error to UI, UI hangs indefinitely | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | `final` event 未发送到 Web UI 导致 spinner 无限挂起；影响所有 Web UI 用户；H 级可用性；已有清晰建议：确保 timeout 时发送 `status:"timeout"` final event |
| P60028 | [#65694](https://github.com/openclaw/openclaw/issues/65694) **(bug)** MCP Server Resource Leak — Orphaned processes cause memory exhaustion and hanging commands | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | 资源泄漏；0评论无标签；未认领 |
| P60029 | [#65687](https://github.com/openclaw/openclaw/issues/65687) **(bug)** Discord voice conversations break after one successful back and forth | 🔍 新发现(方向1 GitHub 第115轮 12:15 CST) | 作者给出精确根因（`resolvePluginCapabilityProviders` 早期返回导致 compat fallback 被跳过）；含修复建议 |
| P60030 | [方向4 第115轮 12:15 CST] PR #65696（chat.abort cascading, size:XS）— greptile P1 + chatgpt P1×2；关键路径：`chatAbortControllers` 在 `reactivateCompletedSubagentSession`/`buildSessionStartupContextPrelude` throw 时留下 stale entry；**author 需修复后才能 merge** | 🔍 **Author 需处理 P1**(方向4 第115轮 12:15 CST) | **持续追踪 merge 时机** |
| P60031 | [方向4 第115轮 12:15 CST] PR #65680（Feishu onboarding streamline, size:XL）— chatgpt P1×4 + greptile P1×3；`accountOverride` 丢失、DM policy top-level only、auth.login ignores cfg、domain written twice；**多账号场景严重 bug，高危** | 🔍 **高危 PR，author 需大量修改**(方向4 第115轮 12:15 CST) | **持续追踪** |
| P60032 | [方向4 第115轮 12:15 CST] PR #65669（cron custom job IDs, size:XL）— greptile P2 + chatgpt P1×3；schema 缺 `id` 字段，`--id` 功能无法 end-to-end 工作；**功能 broken** | 🔍 **功能 broken，author 需修复**(方向4 第115轮 12:15 CST) | **持续追踪** |
| P60033 | [方向4 第115轮 12:15 CST] PR #65700（WhatsApp inbound policy, size:L）— chatgpt P1×2；legacyEntry backfill 写入多余字段，可能导致跨账号数据污染；**P1 需 author 确认** | 🔍 **P1 待 author 确认**(方向4 第115轮 12:15 CST) | **持续追踪 merge 时机** |
| P60034 | [方向4 第115轮 12:15 CST] PR #65702（heartbeat delivery context, size:XS）— greptile P2: variable name shadows outer `entry`；轻微命名问题 | 🔍 新发现(方向4 第115轮 12:15 CST) | 轻微，可快速修复 |

### 当前优先级更新（xixi 第116轮 2026-04-13 13:19 CST）

| P# | Issue | 优先级 | 备注 |
|----|-------|--------|------|
| P60035 | [#65721](https://github.com/openclaw/openclaw/issues/65721) **[Bug]: Beta blocker: OpenClaw Agents Can Self-Destruct Through Configuration Modification** | 🔥 **P0 beta blocker** | agent 修改 `~/.openclaw/openclaw.json` 添加无效字段 `env.shellEnv.vars` 导致 gateway 启动失败；连续两天发生（4-12 Docker sandbox 自毁 + 4-13 配置自毁）；缺乏配置自保机制（写时校验、自动备份）；0评论，未认领；**建议 aoao 优先接单** |
| P60036 | [#65706](https://github.com/openclaw/openclaw/issues/65706) **[Bug]: openclaw nodes status and openclaw nodes list does not sync** | 🔥 **P0 beta blocker regression** | `openclaw nodes list` 永远返回 0 paired，`openclaw nodes status` 显示正常；推理时节点选择错误；所有 paired node 用户受影响；0评论，未认领；**建议 aoao 优先接单** |
| P60037 | [#65701](https://github.com/openclaw/openclaw/issues/65701) **(bug+regression)** Telegram provider sends duplicate messages for every response | 🔥 **高** | v2026.4.5 起所有版本每条响应发两次；独立于 #65078（Telegram transcript 不出现）；1评论；**建议 aoao 接单** |
| P60038 | [#65725](https://github.com/openclaw/openclaw/pull/65725) **fix(browser): return undefined from resolveBrowserSsrFPolicy when unconfigured** | 🔍 XS, maintainer, 有PR | regression 修复：v2026.4.10 安全修复导致 browser plugin 无法连接本地 Chrome CDP；根因清晰；1 maintainer 评论；**建议 review 优先** |
| P60039 | [#65713](https://github.com/openclaw/openclaw/pull/65713) **fix(security): remove busybox/toybox from interpreter-like safe bins** | 🔍 S, maintainer | 安全修复：移除 `INTERPRETER_LIKE_SAFE_BINS` 中的 busybox/toybox，防止 applet 逃逸 exec approval；2 maintainer 评论；**建议 review 优先** |
| P60040 | [#65714](https://github.com/openclaw/openclaw/pull/65714) **fix(approval-auth): prevent empty approver list from granting explicit approval authorization** | 🔍 S, maintainer | 安全修复：空 approver list 意外授予 explicit 授权绕过 isAuthorizedSender；1 maintainer 评论；**建议 review 优先** |
| P60041 | [#65717](https://github.com/openclaw/openclaw/pull/65717) **fix(security): broaden shell-wrapper detection and block env-argv assignment injection** | 🔍 M, maintainer | 安全修复：`sh script.sh` 模式现在也应用 env allowlist；`env SHELLOPTS=x bash` 等被拦截；1 maintainer 评论；**建议 review 优先** |
| P60042 | [#65716](https://github.com/openclaw/openclaw/pull/65716) **fix(anthropic-vertex): resolve model discovery and auth for GCP ADC** | 🔍 S | 修复 GCP ADC 三处问题（.ts→.js fallback、env identity guard、缺失 resolveSyntheticAuth hook）；已有 PR；1 maintainer 评论；**建议 review 优先** |
| P60043 | [#65722](https://github.com/openclaw/openclaw/issues/65722) **[Bug]: openclaw wiki bridge import always returns 0 artifacts — CLI does not initialize memory-core plugin runtime** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | CLI bridge import 调用 `listActiveMemoryPublicArtifacts` 但 `memoryPluginState.capability` 未初始化；根因明确 |
| P60044 | [#65719](https://github.com/openclaw/openclaw/issues/65719) **Control UI: /clear button restores stale history after reset (race condition in ap())** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | `bf(e)` 调用 `chat.history` 导致清空后历史重新加载；根因已定位 dist 文件 |
| P60045 | [#65718](https://github.com/openclaw/openclaw/issues/65718) **Slack inbound mentions stay raw in BodyForAgent / RawBody** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | `<@U09FU0EL0JD>` 未渲染为 `@Bek`；usability gap |
| P60046 | [#65720](https://github.com/openclaw/openclaw/issues/65720) **[Feature]: WebUI should remain accessible after Gateway crashes** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | 功能请求；gateway crash 时 fallback 到静态页 |
| P60047 | [#65724](https://github.com/openclaw/openclaw/issues/65724) **[Windows] DeprecationWarning: Passing args to child process with shell option true** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | Windows 上 Node.js DEP0190 警告；`shell: true` 安全问题 |
| P60048 | [#65723](https://github.com/openclaw/openclaw/issues/65723) **Discord exec approvals should poll DM for responses instead of timing out** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | Discord DM 通知延迟导致 approve 超时；建议轮询 |
| P60049 | [#65726](https://github.com/openclaw/openclaw/issues/65726) **ACP bindings: add notifyPolicy config + dedup guard for bound threads** | 🔍 新发现(方向1 GitHub 第116轮 13:19 CST) | ACP task completion 通知泄露 Slack；无 config 方式设置 notifyPolicy |
| P60050 | [方向2 Tencent/openclaw-weixin 第116轮 13:19 CST] 过去2小时无新更新；已覆盖 issues #57/#56/#55/#54/#53/#52/#51/#50 | 📋 无新发现(方向2 插件 第116轮 13:19 CST) |  |
| P60051 | [方向3 贡献者文件区 第116轮 13:19 CST] 扫描末段15位贡献者（9-12次贡献）：joelnishanth/xinhuagu/MoerAI/aether-ai-agent/chilu18/emonty/jadilson12/davemorin 等；gh API 返回空；无新发现 | 📋 无新发现(方向3 第116轮 13:19 CST) |  |

### 当前优先级更新（xixi 第117轮 2026-04-13 14:23 CST）

| P# | Issue | 优先级 | 备注 |
|----|-------|--------|------|
| P60052 | [#65721](https://github.com/openclaw/openclaw/issues/65721) **[Bug]: Beta blocker: OpenClaw Agents Can Self-Destruct Through Configuration Modification** | 🔥 **P0 beta blocker** | 安全+crash；agent 修改 `openclaw.json` 添加无效字段导致 gateway 启动失败；连续两天发生；缺乏配置自保机制；0评论，未认领；**建议 aoao 立即接单** |
| P60053 | [#65740](https://github.com/openclaw/openclaw/issues/65740) **[Bug]: Chrome CDP websocket unreachable after start - macOS ARM64** | 🔥 **高 regression** | macOS ARM64 Chrome CDP 连接失败；regression 标签；0评论；与已知 CDP 问题簇重叠；**建议 aoao 接单** |
| P60054 | [#65733](https://github.com/openclaw/openclaw/issues/65733) **[Bug]: Plugin discovery via safeRealpathSync takes ~20 minutes during gateway startup on Windows** | 🔥 **高 bug:behavior** | Windows 启动延迟 20 分钟；影响所有 Windows 用户；0评论未认领；**建议 aoao 接单** |
| P60055 | [#65744](https://github.com/openclaw/openclaw/issues/65744) **[Bug]: Feishu DM response truncated and leaked to group chat — similar to #55019** | 🔥 **高** | 飞书 DM 回复截断并泄露到群聊；类似历史问题 #55019；0评论；**建议 aoao 接单** |
| P60056 | [#65728](https://github.com/openclaw/openclaw/issues/65728) **[Bug]: always get unauthorized when I start openclaw gateway run** | 🔍 S regression | 认证失败；regression 标签；0评论 |
| P60057 | [#65747](https://github.com/openclaw/openclaw/issues/65747) **[Bug]: 钉钉官方插件无法工作 - Stream 模式接入失败** | 🔍 S | 钉钉插件 Stream 模式失败；0评论 |
...
| P60068 | [#65843](https://github.com/openclaw/openclaw/issues/65843) **P1 (bug:behavior)** ACP agent pairing: `unknown requestId` — block ACP setup，100%复现 | 🔥 **新发现**(方向1 第115轮 17:53 CST) | 根因：ACP 握手阶段 requestId 不匹配；影响所有 ACP 远程用户；0评论未认领；**建议立即派出修复** |
| P60069 | [#65835](https://github.com/openclaw/openclaw/issues/65835) **🔒 P1** `sourceConfig` in `config.get` exposes API keys in plaintext | 🔥 **新发现**(方向1 第115轮 17:53 CST) | **安全漏洞**；sourceConfig 返回明文 secrets 而 parsed/resolved 已脱敏；任何可调用 config.get 的组件都能读取凭证；**建议 aoao 优先接单** |
| P60070 | [#65840](https://github.com/openclaw/openclaw/issues/65840) **P1 (bug+regression)** image tool 报错 `Unknown model` despite correct registration | 🔍 **新发现**(方向1 第115轮 17:53 CST) | regression；imageModel 配置正确但 image 工具路由失败；影响所有图片用户；0评论；**建议 aoao 接单** |
| P60071 | [#65834](https://github.com/openclaw/openclaw/issues/65834) **P2 (bug)** memory-core dreaming cron reconciliation 误报 `cron service unavailable` | 🔍 **新发现**(方向1 第115轮 17:53 CST) | runtime cron service reference resolution bug；0评论；与 P59907(#55679) 相关但不同触发条件；**建议 aoao 接单** |
| P60072 | [#65838](https://github.com/openclaw/openclaw/issues/65838) **S** Windows Bonjour/mDNS 无Bonjour时启动慢50秒 | 🔍 **新发现**(方向1 第115轮 17:53 CST) | 功能请求：加配置关闭 mDNS advertising；S 级；**建议 aoao 接单** |
| P60073 | [#65899](https://github.com/openclaw/openclaw/issues/65899) **P0 (external)** Hook error: Cannot access group before initialization in message:sent | ✅ **非 openclaw 问题**(方向1 第119轮 19:35 CST) | root cause: third-party clawhub managed hooks (clawvault/telegram-stickers) — NOT in openclaw/openclaw repo, no PR possible; user can disable hooks one by one to isolate |
| P60074 | [#65902](https://github.com/openclaw/openclaw/issues/65902) **P1** WhatsApp session discovery inconsistent with runtime status | 🔍 **新发现**(方向1 第119轮 19:09 CST) | session discovery 与 `openclaw status` 不一致；cron 能发但 agent 路由找不到；**建议 aoao 接单** |
| P60075 | [#65890](https://github.com/openclaw/openclaw/issues/65890) **P1 (bug+regression)** QMD backend falls back to builtin on live memory search | 🔍 **新发现**(方向1 第119轮 19:09 CST) | 集合名 `memory-alt-main` vs `memory-root-main` 不匹配；请求 QMD 但 fallback 到 builtin；**建议 aoao 接单** |
| P60076 | [#65883](https://github.com/openclaw/openclaw/issues/65883) **P2** webchat: queued user message vanishes from chat history after dequeue | 🔍 **新发现**(方向1 第119轮 19:09 CST) | 队列消息在 dequeue 后气泡消失；transcript 不完整；**建议 aoao 接单** |
| P60077 | [#65875](https://github.com/openclaw/openclaw/issues/65875) **P2** Session model override not cleared on /new | 🔍 **新发现**(方向1 第119轮 19:09 CST) | `/new` 后 model override 未清除；persists 到新 session；**建议 aoao 接单** |
| P60078 | [#65847](https://github.com/openclaw/openclaw/issues/65847) **🔥 P0** `agents.defaults.heartbeat.to` corrupts deliveryContext — 所有消息错误路由到心跳目标 | 🔥 **新发现**(方向1 第120轮 19:08 CST) | **严重隐私+路由 bug**：配置 heartbeat.to 后所有 DM 都被路由到心跳目标渠道，用户私信全部泄漏；0评论无标签；S-M 可修性；**建议 aoao 立即接单** |
| P60079 | [#65867](https://github.com/openclaw/openclaw/issues/65867) **P1 (bug+regression)** Gemini `<final>` tags leak into delivered messages | 🔍 **新发现**(方向1+方向3 第120轮 19:08 CST) | regression；Gemini 思考标签漏出到交付消息；**MoerAI 文件区（assistant-visible-text.ts）直接命中**；bug 标签，0评论；**建议 aoao 接单** |
| P60080 | [方向4 第120轮 19:08 CST] PR #65577 **已 MERGE** ✅；PR #65206 关闭（未 merge，需确认替代 PR）；PR #65021 关闭（未 merge）；PR #65574 生产确认修复有效（@mjamiv 确认），接近 merge；PR #65511 @100yenadmin 呼吁 @steipete @vincentkoc review；PR #65016 有 CHANGES_REQUESTED | 👀 状态变化(方向4 第120轮 19:08 CST) | #65577 merge 是本轮最大进展；#65206 关闭需跟进替代 PR；#65574 持续接近可 merge |

| P60081 | [#66135](https://github.com/openclaw/openclaw/issues/66135) **🔥 regression** Background exec exit 不唤醒 session、cron --at jobs 静默消失、Claude CLI --print 输出截断（2026.4.10） | 🔥 **建议 aoao 立即接单**(方向1 GitHub 第122轮 15:19 CST) | **最高优先级**。三重 regression 同时出现：background exec exit 不唤醒 session、cron --at jobs 静默消失、Claude CLI --print 被截断。2026.4.10 引入，有 gateway logs 证据。Size S，独立可修。**建议 aoao 优先接单** |
| P60082 | [#66393](https://github.com/openclaw/openclaw/issues/66393) **regression** webchat 图片粘贴/附件无法被 assistant 分析 | 🔍 新发现(方向1 GitHub 第122轮 15:19 CST) | 图片粘贴到 webchat 后 assistant 无法查看；所有其他 AI assistant 均正常；**建议 aoao 接单** |
| P60083 | [#66391](https://github.com/openclaw/openclaw/issues/66391) **regression** TUI tool calls fail "session ID openclaw-tui does not exist" on local Ollama | 🔍 新发现(方向1 GitHub 第122轮 15:19 CST) | TUI session 路由问题，本地模型工具调用 100% 失败；**建议 aoao 接单** |
| P60084 | [#43767](https://github.com/openclaw/openclaw/issues/43767) **regression** Heartbeat ignores lightContext: true，加载完整 context + unbounded history | 🔍 新发现(方向1 GitHub 第122轮 15:19 CST) | 心跳机制忽略 lightContext，每次 tick 加载完整 context 导致 token 无界增长；4 条评论；**建议 aoao 接单** |
| P60085 | [#66389](https://github.com/openclaw/openclaw/issues/66389) **regression** acpx embedded runtime 每个消息 spawn 新进程，cache hit rate 100%→35%（2026.4.11） | 🔍 新发现(方向1 GitHub 第122轮 15:19 CST) | acpx embedded runtime 进程复用失效，cost 激增；**建议 aoao 接单** |
| P60086 | [Tencent/openclaw-weixin PR #63](https://github.com/Tencent/openclaw-weixin/pull/63) fix(inbound): preserve quoted context for voice messages with ref_msg | 🔍 新发现(方向2 插件 第122轮 15:19 CST) | +214/-2，2 files；修复 voice 消息引用上下文丢失；**建议 review** |
| P60087 | [Tencent/openclaw-weixin PR #62](https://github.com/Tencent/openclaw-weixin/pull/62) feat: support sending voice messages (SILK/MP3/OGG) | 🔍 新发现(方向2 插件 第122轮 15:19 CST) | +83/-2，4 files；新增原生语音消息发送；**建议 review** |
| P60088 | [Tencent/openclaw-weixin #61](https://github.com/Tencent/openclaw-weixin/issues/61) message uploadFileAttachmentToWeixin: getUploadUrl returned no upload URL | 🔍 新发现(方向2 插件 第122轮 15:19 CST) | 文件上传失败；⚠️ 代码不可见 |
| P60089 | [方向3 第122轮 15:19 CST] lml2468 最近 commit：fix(gateway): plug long-running memory leaks（sha 36c3a54b） | 🔍 新发现(方向3 贡献者 第122轮 15:19 CST) | contributor 在修 gateway 内存泄漏；与 P59907(#55679 memoryFlush)方向吻合；**建议 aoao 比对是否已在主仓修复** |
| P60090 | [方向4 第122轮 15:19 CST] PR #65577 **已 MERGE** ✅；PR #65574 持续 5 个 fix commits 接近可 merge；PR #65758 WhatsApp reconnect safety timer +124/-0 活跃 review | 👀 状态变化(方向4 第122轮 15:19 CST) | #65577 merge 是重大进展；#65574 关注 merge 时机；#65758 是活跃 WhatsApp 质量改进 |
| P60101 | [#66573](https://github.com/openclaw/openclaw/issues/66573) **S** chat.history limit=2000 exceeds server maximum of 1000, no retry | 🔥 **建议派 aoao 接单**(方向1 GitHub 第124轮 21:50 CST) | 客户端写死 limit=2000，服务器上限 1000，收到 INVALID_REQUEST 后静默跳过不重试；历史上下文丢失；0评论；根因清晰，S级可修性好；**建议立即接单** |
| P60102 | [#66564](https://github.com/openclaw/openclaw/issues/66564) **S regression** Control UI feels significantly laggier in 2026.4.12 | 🔥 **建议派 aoao 接单**(方向1 GitHub 第124轮 21:50 CST) | 2026.4.12 regression；eager connect-time 加载 + 多频轮询导致 UI 卡顿；bundle audit 证据充分；与 #66575 相关但独立；**建议派 aoao 接单** |
| P60103 | [#66561](https://github.com/openclaw/openclaw/issues/66561) **S** openai-codex SSE stream begins but aborts locally, surfaced as 408 timeout | 🔍 建议派 aoao 接单(方向1 GitHub 第124轮 21:50 CST) | mitmproxy 证明 upstream 已开始响应（first response byte 12:03:01.557），但 client 端 12:03:02.215 abort；被误分类为 timeout 导致不必要的 failover；证据链完整；**建议派 aoao 接单** |
| P60104 | [#66558](https://github.com/openclaw/openclaw/issues/66558) **S** sessions_spawn/sessions_send not available in main agent despite tools.profile: "coding" | 🔍 建议派 aoao 接单(方向1 GitHub 第124轮 21:50 CST) | `group:sessions` 工具在 main agent 不可用，尽管 global tools.profile=coding；日志警告 `tools.allow allowlist contains unknown entries`；0评论；**建议派 aoao 接单** |
| P60105 | [#66553](https://github.com/openclaw/openclaw/issues/66553) **M** Race condition — voice message fires before STT transcription available | 🔍 建议先调研(方向1 GitHub 第124轮 21:50 CST) | WhatsApp voice → StoreMessage 提交 transcription=NULL → agent turn 触发 → agent 读到空 → STT 2-5秒后完成但 turn 已结束；100% 可复现；建议先调研根因路径再派单 |
| P60106 | [#66522](https://github.com/openclaw/openclaw/issues/66522) **M** Session index rebuilds on Gateway restart, session history completely lost | 🔍 新发现(方向1 GitHub 第124轮 21:50 CST) | sessions.json 重建但 session 文件存在；sessions_list API 只返回 ended=false 的 session；无 warning；无备份；严重影响长期使用用户 |
| P60107 | [#66535](https://github.com/openclaw/openclaw/issues/66535) **S** /compact command cannot be canceled while in progress | 🔍 建议派 aoao 接单(方向1 GitHub 第124轮 21:50 CST) | /compact 运行中无法取消，用户体验影响大；0评论未认领；**建议派 aoao 接单** |
...
| P60116 | [#66535](https://github.com/openclaw/openclaw/issues/66535) **S** /compact command cannot be canceled while in progress | ⚠️ **修复失败，重新派出**(方向1 GitHub 第124轮 14:45 CST) | 首次尝试：subagent 找到根因（缺少 abortSignal + 未注册到 ACTIVE_EMBEDDED_RUNS），创建 PR #66616 后 session gateway 关闭导致失败；已重新派出（runId ad5f2e4b） |
| P60117 | [#66558](https://github.com/openclaw/openclaw/issues/66558) **S** sessions_spawn/sessions_send not available in main agent despite tools.profile: coding | ⚠️ **修复失败，重新派出**(方向1 GitHub 第124轮 14:50 CST) | 首次尝试 subagent gateway 关闭（14:32 CST，13分钟运行但无 PR 回传）；已重新派出（runId 379a000e） |
...
| P60201 | [#66626](https://github.com/openclaw/openclaw/issues/66626) **🔒 S security** config.get leaks raw secrets via sourceConfig/runtimeConfig paths | 🔥 **建议 aoao 立即接单**(方向1 GitHub 第126轮 00:06 CST) | 🔒安全漏洞；parsed/resolved 正确 redact，但 sourceConfig/runtimeConfig 返回明文；任何 skill/plugin/LLM turn 可读取全部 API key/token 明文；影响环境变量密钥注入 LLM context；**最高安全优先级**；0评论无标签 |
| P60202 | [#66647](https://github.com/openclaw/openclaw/issues/66647) **S regression** Telegram: Binary file content injected into prompt via msg.caption causes token explosion | 🔥 **建议 aoao 接单**(方向1 GitHub 第126轮 00:06 CST) | regression（2026.4.12/14）；.mobi 文件 → 460,506 tokens → context overflow；根因：getTelegramTextParts() 无 binary content 检测；作者已给完整 fix 建议；LINE 渠道有参照实现 |
| P60203 | [#66646](https://github.com/openclaw/openclaw/issues/66646) **S** Session file lock errors cascade through model fallback chain, wasting 30s per incident | 🔥 **建议 aoao 接单**(方向1 GitHub 第126轮 00:06 CST) | 46% fallback decision 实际是锁争用（非 model failure）；每次浪费 10s×3 models=30s；多 agent+cron 结构性问题；根因：锁错误应归类为基础设施错误，不触发 fallback |
| P60204 | [#66641](https://github.com/openclaw/openclaw/issues/66641) **S** Installer crashes after "Select channel (QuickStart)" with TypeError: Cannot read properties of undefined (reading 'trim') | ✅ **PR #66649 已覆盖**(方向1 GitHub 第126轮 00:06 CST) | 阻断所有新用户安装（2026.4.14）；**好消息：PR #66649 fix(wizard): avoid setup crash on missing provider ids 已覆盖，等 merge** |
| P60205 | [#66635](https://github.com/openclaw/openclaw/issues/66635) **S regression** WhatsApp auto-reply inline MEDIA:/absolute/path fails, manual message send --media succeeds | 🔥 **建议 aoao 接单**(方向1 GitHub 第126轮 00:06 CST) | regression（2026.4.14）；同一文件手动发送成功但 assistant auto-reply 失败；说明不同投递路径；与 #66535 不同（/compact cancel）；**建议 aoao 接单** |
| P60206 | [#66633](https://github.com/openclaw/openclaw/issues/66633) **S regression** openai-codex/gpt-5.4 fails with Cloudflare 403 on every request (2026.4.12 → 2026.4.14) | 🔍 待确认修复状态(方向1 GitHub 第126轮 00:06 CST) | regression；2026.4.12 正常，2026.4.14 每请求必 403；根因：originator: pi + User-Agent: pi 被 Cloudflare 拦截；所有 headless 自托管用户受影响；确认是否有已有 PR 在修 User-Agent 头 |
| P60207 | [#66657](https://github.com/openclaw/openclaw/issues/66657) **S** TypeError on Feishu group messages with empty/undefined content (pure @mention) | 🔍 **作者已给 fix**(方向1 GitHub 第126轮 00:06 CST) | 根因：sequential-key.ts 和 sequential-queue.ts 调用 .content?.trim() 未做可选链；作者已给两行可选链 fix；**建议 aoao 接单，1行可修** |
| P60208 | [#66639/#66625](https://github.com/openclaw/openclaw/issues/66639) **S** MiniMax provider not registered as media-understanding provider, image tool cannot use MiniMax | 🔍 新发现(方向1 GitHub 第126轮 00:06 CST) | 同根：两条 issue（中文+英文）描述同一问题；MiniMax API 本身支持图片理解，但 OpenClaw 未注册 media-understanding provider；Workaround：用 Google 模型 |
| P60209 | [#66631](https://github.com/openclaw/openclaw/issues/66631) **M** Feishu: Bot replies create new topics instead of replying within existing topic | 🔍 新发现(方向1 GitHub 第126轮 00:06 CST) | Feishu topic 模式对话中 bot 不回复现有 topic 而创建新 topic；无标签无评论；影响 topic 会话一致性；**建议 aoao 接单** |
| P60210 | [#66648](https://github.com/openclaw/openclaw/issues/66648) **M** System-level Exec completed/failed notifications leak into unrelated webchat sessions | 🔍 **建议 aoao 接单**(方向1 GitHub 第126轮 00:06 CST) | session targeting bug；其他 session 的 exec 事件被路由到当前 webchat session；用户体验 bug + token 浪费；引用 #25959 历史相关；**建议 aoao 接单** |
| P60211 | [Tencent/openclaw-weixin #65/#64/#61](https://github.com/Tencent/openclaw-weixin/issues/65) **S** 微信新 issue：消息乱序(#65)、文件上传失败(#61)、支持好友/群发送(#64 feature) | 🔍 新发现(方向2 插件 第126轮 00:06 CST) | 近24小时内新 issue；#65 消息乱序可能与 openclaw core message ordering 相关；#61 文件上传与已有 issue 簇相关；**建议跟进** |
| P60212 | [Tencent/openclaw-weixin PR #63/#62/#49](https://github.com/Tencent/openclaw-weixin/pull/63) **S** 微信插件新 PR：voice message ref_msg 修复(#63/#49)、voice send 支持(#62) | 🔍 新发现(方向2 插件 第126轮 00:06 CST) | #63 fix(inbound): preserve quoted context for voice messages with ref_msg — 与 #66639 相关（语音消息上下文）；#62 feat: support sending voice messages — 新功能；**建议跟进** |
| P60213 | [方向4 第126轮 00:06 CST] **无 maintainer 人工新评论**；**PR #66649 新开覆盖 #66641**；**PR #66636 maintainer size:L 持续活动**（workspace file opens 安全收紧） | 👀 状态变化(方向4 第126轮 00:06 CST) | 本轮无 maintainer 人工新评论；关注 #66649 merge 时机；#66636（maintainer L）安全相关，持续跟进；#66583/#66580/#66575/#66574 新开 feature/bug PR |
| P60214 | [#66650](https://github.com/openclaw/openclaw/pull/66650) fix(tools): clamp sessions_history limit to chat.history max | ✅ **PR 已覆盖 #66573**(方向1 第126轮 00:06 CST) | **直接覆盖 P60101**（#66573 chat.history limit=2000 exceeds server max 1000，静默丢失上下文）；PR #66650 将 limit 提升到 2000，等 merge |
| P60215 | [#66623](https://github.com/openclaw/openclaw/pull/66623) Fix #66535: wire up abort signal for /compact command | ✅ **PR 已覆盖 P60106**(方向1 第126轮 00:06 CST) | **直接覆盖 P60106**（/compact 无法取消）；作者已提 PR，等 review |
| P60216 | [#66601](https://github.com/openclaw/openclaw/issues/66601) **🔥 S regression blocker** `lossless-claw` context engine 工厂在 v2026.4.14 返回 invalid ContextEngine，导致每次 turn 都 Embedded agent failed | 🔥 **建议 aoao 立即接单**(方向1 GitHub 第127轮 00:34 CST) | regression（v2026.4.14 in-place upgrade from 4.11）；`info.id must match registered id "lossless-claw"` 验证失败；插件正常加载，错误在 factory consumer 端校验 regression；全部 session 100% 失败；滚动回 4.11 立即恢复；3 条评论确认；**无现有 PR** |
| P60217 | [#66669](https://github.com/openclaw/openclaw/issues/66669) **S** Control UI (webchat) 不渲染用户发送的图片附件 | 🔥 **建议 aoao 接单**(方向1 GitHub 第127轮 00:34 CST) | 前端 bundle user message 组件未渲染 `attachments` 数组；AI 消息图片正常，用户消息只显示空 bubble；后端/Gateway 正确（AI 能看到图片）；根因清晰（user message component 缺附件渲染）；**无现有 PR** |
| P60218 | [#66668](https://github.com/openclaw/openclaw/issues/66668) **S** `plugins uninstall` 对 `--link`/手动安装的插件不删除文件 | 🔍 **作者已给 fix 路线**(方向1 GitHub 第127轮 00:34 CST) | uninstall 只清理 `plugins.installs`（npm）的文件；`--link`/手动复制不在其中；作者已给两条修复路线；无现有 PR；**建议 aoao 接单** |
| P60219 | [Tencent/openclaw-weixin #65](https://github.com/Tencent/openclaw-weixin/issues/65) **S** 微信消息接收乱序：网页端能收到微信收不到 | 🔍 新发现(方向2 插件 第127轮 00:34 CST) | 用户证据充分（截图对比 webchat vs 微信）；消息内容正确但微信端漏收；可能是微信长连接保活/idle timeout 机制问题；1 条 +1 确认；**建议 aoao 调研** |
| P60220 | [Tencent/openclaw-weixin #61](https://github.com/Tencent/openclaw-weixin/issues/61) **S** 发送文件报错：`uploadFileAttachmentToWeixin: getUploadUrl returned no upload URL` | 🔍 新发现(方向2 插件 第127轮 00:34 CST) | 文件上传失败；长期 open 无 maintainer 响应（1 条评论）；**建议 aoao 调研** |
...
- **inProgressFixes**：
  - #66283 @ 12:00 — cron agentTurn crash，aoao 已派出（XS，2行 fix）
- **最新扫描摘要**（11:21 CST）：
  - **#66283** 🔥 cron agentTurn payload crashes（XS，2行 guard fix）
...
| P60221 | [#66849](https://github.com/openclaw/openclaw/issues/66849) | 🔥 **S regression**: active-memory pre-reply timeout → 全面会话失败 (2026.4.14) | 升级后所有现有会话全面失败；回滚 2026.4.12 恢复；影响所有渠道多模型；根因在 active-memory pre-reply timeout；0 评论未认领；**派出 fix** |
| P60222 | [#66822](https://github.com/openclaw/openclaw/issues/66822) | 🔥 **S crash**: memorySearch.concurrency=1 配置被忽略 → Ollama 被灌爆 → SIGKILL | 设 concurrency=1 后仍 30+ 并发 embedding 请求 → 进程被 kill，0 chunks indexed；crash；根因在 concurrency 逻辑；0 评论；**派出 fix** |
| P60223 | [#66830](https://github.com/openclaw/openclaw/issues/66830) | 🟠 **S**: Compaction dead zone — reserveTokens 调整无效 (vs reserveTokensFloor) | memoryFlush threshold 对 floor 而非实际 reserveTokens 计算；长运行会话卡在 fallback 模型；参照 #66845 (flush dedup) 是否同根；0 评论；**派出 fix** |
| P60224 | [#66848](https://github.com/openclaw/openclaw/issues/66848) | 🟠 **S crash**: TypeError: Cannot read properties of undefined (reading 'trim') | crash；同根因已在多个 PR 修过（#66653）；1 行可选链 fix；0 评论；**派出 fix** |
| P60225 | [#66832](https://github.com/openclaw/openclaw/issues/66832) | 🔒 **S security**: logs.tail credential redaction — 5 类格式未覆盖 | JWT、X-OpenClaw-Token、x-pomerium-jwt-assertion、Basic auth 等通过 logs.tail 泄漏给 operator.read 客户端；根因在 redaction regex；**派出 fix** |
| P60226 | [#66839](https://github.com/openclaw/openclaw/issues/66839) | 🟠 **S regression**: macOS node 不 advertise system.run.prepare — exec host=node 完全失效 | macOS 节点 exec host=node 100% 失败；capability advertisement 路径缺失；**派出 fix** |
| P60227 | [#66828](https://github.com/openclaw/openclaw/issues/66828) | 🟠 **S regression**: Cron lane 被 remap 为 Nested + subagent cleanup packaging 失效 (2026.4.12) | cron.maxConcurrentRuns 配置被忽略（lane 变 concurrency 1）；subagent cleanup 失败；**派出 fix** |
| P60228 | [#66813](https://github.com/openclaw/openclaw/issues/66813) | 🟠 **S**: webchat heartbeat target="last" 被 skip — 阻塞 background execution | webchat 直接会话心跳被 skip；background execution 完全阻塞；target last 逻辑 bug；**派出 fix** |
| P60229 | [#66847](https://github.com/openclaw/openclaw/pull/66847) | 🔒 **S security PR**: guard instruction file writes from agent sessions | wkeything 安全修复 PR；防止 agent session 覆写 SOUL.md/CLAUDE.md 等安全约束文件；**建议 maintainer review + merge** |
| P60230 | [#66842](https://github.com/openclaw/openclaw/pull/66842) | ✅ **PR 已开覆盖 #66833**: fix(agents) OpenRouter reasoning_details stream parser | bde1 PR 修复 OpenRouter/Qwen3 reasoning_details 字段未处理导致 payloads=0；**覆盖 #66833 同根因，无需重复追踪** |
| P60231 | [#66849](https://github.com/openclaw/openclaw/issues/66849) | 🔴 **S regression**: active-memory pre-reply timeout 导致升级后所有渠道现有会话全面失败 (2026.4.14) | 升级后 Feishu/Weixin 等所有渠道多模型多 surface 现有会话全面失败；回滚到 2026.4.12 恢复；根因：active-memory pre-reply 路径反复超时；broad failure 影响所有现有用户；**建议 aoao 立即派出 fix** |
| P60232 | [#66822](https://github.com/openclaw/openclaw/issues/66822) | 🔴 **S crash**: memorySearch.remote.batch.concurrency=1 配置被忽略 — SIGKILL 终止进程 | 设 concurrency=1 后 OpenClaw 仍发 30+ 并发 embedding 请求 → Ollama 被灌爆 → SIGKILL 终止进程，0 chunks indexed；配置被完全忽略；**建议 aoao 派出 fix** |
...
| P60187 | [#67252](https://github.com/openclaw/openclaw/issues/67252) **S** feishu_doc write action fails to clear all blocks on large documents (missing pagination) | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | `clearDocumentContent` 无循环处理大文档，缺少分页逻辑；根因清晰，S级 fix；**建议立即接单** |
| P60188 | [#67251](https://github.com/openclaw/openclaw/issues/67251) **🔴 S regression** Windows CLI subcommands hang with SIGKILL in exec environment (2026.4.14) | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | Windows exec 环境 CLI 子命令挂死并收到 SIGKILL；2026.4.14 regression；**建议 aoao 接单** |
| P60189 | [#67250](https://github.com/openclaw/openclaw/issues/67250) **S regression** Control UI: streaming text disappears when tool calls display is hidden | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | 4.14 regression；streaming 文本在工具调用隐藏时消失；**建议 aoao 接单** |
| P60190 | [#67248](https://github.com/openclaw/openclaw/issues/67248) **S** sessions_spawn(runtime="subagent") still fails on 2026.4.14 with ACP-only streamTo under GPT-5.4 | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | 2026.4.14 仍未修复；ACP-only streamTo 场景；**建议 aoao 接单** |
| P60191 | [#67247](https://github.com/openclaw/openclaw/issues/67247) **🟠 S regression** [Telegram] Native command menu disappears after upgrade to 4.14 | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | 4.14 regression；命令菜单完全消失；**建议 aoao 接单** |
| P60192 | [#67241](https://github.com/openclaw/openclaw/issues/67241) **🟠 S** BlueBubbles attachment downloads fail silently on Node 22.20+ — invalid onRequestStart method | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | Node 22.20+ 上 invalid onRequestStart method 导致静默失败；**建议 aoao 接单** |
| P60193 | [#67254](https://github.com/openclaw/openclaw/pull/67254) **fix(google)** respect allowPrivateNetwork config for SSRF policy | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | Joel-Claw PR，0 comments，5分钟前新开；SSRF policy 修复；**建议关注 merge 时机** |
| P60194 | [#67255](https://github.com/openclaw/openclaw/pull/67255) **feat** add yuanbao extension plugin | 🔍 **新发现**(方向1 GitHub 第133轮 22:54 CST) | loongfay PR，1 comment，1分钟前最新创建；新插件功能；**建议关注** |
...
| P60196 | [#69296](https://github.com/openclaw/openclaw/issues/69296) **🔴 S regression** ACP client 在 2026.4.15 中 hang，slash command 和 prompt 全部失效 | ⚠️ **已调研，未产出 PR** (runId 2672af82, 22min) | 根因分析：聚焦 trace_level config option 和 SDK 版本差异，但未能定位具体 crash root cause；建议由 main 自行跟进或重新派出更 targeted 的调查 |
| P60197 | [#69294](https://github.com/openclaw/openclaw/issues/69294) **🔴 S** ACP plugin 在 Windows 11 上导致 gateway 崩溃 | 🔍 新发现 | 待接单 |
| P60198 | [#69286](https://github.com/openclaw/openclaw/issues/69286) **🔴 S** Compaction 后 totalTokens 不重置，触发无限 safeguard loop | ✅ **PR created** ([#69393](https://github.com/openclaw/openclaw/pull/69393) fix/69300-compaction-memory-flush) | fix: resetUsageTotals() after compaction in both retry/non-retry paths; 2 new tests in wired-hooks-compaction.test.ts |
