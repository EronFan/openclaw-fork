...
| P60121 | [#66647](https://github.com/openclaw/openclaw/issues/66647) **🔥 S regression** Telegram 二进制文件 caption 注入 LLM prompt，导致 token 爆炸 | 🔍 新发现(方向1 GitHub 第126轮 00:06 CST) | `.mobi` 电子书文件 → 460,506 tokens → context overflow；根因：`getTelegramTextParts()` 无 binary content 检测；LINE 渠道有正确实现可参照；**作者已给完整 fix 建议；建议 aoao 接单** |
