...
| P60129 | [#66601](https://github.com/openclaw/openclaw/issues/66601) **🔥 regression blocker** `lossless-claw` context engine 工厂返回 invalid ContextEngine，系统彻底不可用 | 🔥 **aoao 已派出**(方向1 GitHub 第126轮 00:34 CST) | v2026.4.14 regression；`info.id must match registered id "lossless-claw"` 验证失败；全部 session 每次 turn `candidate_failed`→`Embedded agent failed before reply`；滚动回 4.11 问题立即消失；多用户 confirmed（3条评论）；**无现有 PR；最高优先级** |
