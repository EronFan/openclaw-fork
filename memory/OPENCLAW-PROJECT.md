...
| P60130 | [#66669](https://github.com/openclaw/openclaw/issues/66669) **S** Control UI (webchat) 不渲染用户发送的图片附件，只显示空 bubble | 🔍 新发现(方向1 GitHub 第126轮 00:34 CST) | 前端 UI regression；user message 组件未渲染 `attachments` 数组；AI 消息图片正常；**无现有 PR；根因清晰** |
| P60131 | [#66668](https://github.com/openclaw/openclaw/issues/66668) **S** `plugins uninstall` 对 `--link` 或手动安装的插件不删除文件 | 🔍 新发现(方向1 GitHub 第126轮 00:34 CST) | 功能缺口；`uninstall` 只清理 npm 安装文件；`--link`/手动复制不在其中；**作者已给两条修复路线** |
