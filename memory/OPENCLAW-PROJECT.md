...
| P60200 | [#69343](https://github.com/openclaw/openclaw/issues/69343) **S** Telegram CLI `message thread create` 错发 `thread-create` 导致 forum topic 创建失败 | 🔥 **aoao 已派出**(runId 3096138c 第134轮 21:41 CST) | Telegram runtime 已支持 `topic-create`，但通用 CLI bridge 未做 `thread-create -> topic-create` 归一化；100% 可复现，根因清晰，属 channel usability gap；**已按 xixi 最新扫描结论派 aoao 修复** |
| P60201 | [#69341](https://github.com/openclaw/openclaw/issues/69341) **XS docs/CLI gap** `openclaw channels status` 文案提示 `--deep`，实际 flag 不存在 | 🔍 **新发现**(方向1 GitHub 第134轮 21:37 CST) | help / tip 与 CLI 实现不一致；不是 crash，但会误导排障；可用 1 个 docs/CLI 小修补齐 |
